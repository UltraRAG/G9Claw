...
public static class AppTabCatalog
{
    ...
    public static readonly IReadOnlyList<AppTab> GeneralProjectTabs =
    [
        AppTab.Chat,
        AppTab.Skills,
    ];

    public static readonly IReadOnlyList<V2TabDescriptor> GeneralProjectTabDescriptors =
    [
        PrimaryTabDescriptors.First(descriptor => descriptor.Tab == AppTab.Chat),
        PrimaryTabDescriptors.First(descriptor => descriptor.Tab == AppTab.Skills),
    ];
    ...
    public static IReadOnlyList<V2TabDescriptor> ToolSwitcherTabsForProject(WorkspaceProject? project)
    {
        if (project is null)
        {
            return [Descriptor(AppTab.Chat)];
        }

        return V2SidebarProjection.IsGeneralProject(project)
            ? GeneralProjectTabDescriptors
            : PrimaryTabDescriptors;
    }

    public static AppTab ClampToolSwitcherTab(AppTab activeTab, WorkspaceProject? project)
    {
        var available = ToolSwitcherTabsForProject(project);
        return available.Any(tab => tab.Tab == activeTab) ? activeTab : AppTab.Chat;
    }
}
...
public static class GeneralProjectEntryPresentation
{
    ...
    public static bool ShouldRender(
        WorkspaceProject? selectedProject,
        AppTab activeTab,
        SidebarSection sidebarSection,
        bool hasMessages,
        bool isReadOnlyBackgroundSession) =>
        activeTab == AppTab.Chat &&
        sidebarSection == SidebarSection.General &&
        !hasMessages &&
        !isReadOnlyBackgroundSession &&
        ShouldRender(selectedProject);
