...
public static class AgentToolNameCanonicalizer
{
    public static string Canonical(string rawName)
    {
        ...
        if (lower.StartsWith("pilotdeck-rag:", StringComparison.Ordinal)) return trimmed;
...
public static class AgentToolRegistry
{
    ...
    [
        ...
        Tool("Skill", "Load a PilotDeck skill. Use pilotdeck-rag:glm-web-search for public web search and weather, or pilotdeck-rag:rag-research for source-grounded research.",
            Props(("skill", Str("Skill name, for example pilotdeck-rag:glm-web-search.")), ("args", Str("User query or task arguments for the skill."))), ["skill"]),
    ...
    private static NormalizedInvocation? CanonicalizedLegacySearchInvocation(string callId, string toolName, string inputJson)
    {
        ...
        if (string.IsNullOrWhiteSpace(rawQuery))
        {
            return new NormalizedInvocation(
                ...
                new AgentToolResult(callId, "Skill", $"{toolName} is disabled. Use Skill with pilotdeck-rag:glm-web-search and provide a query.", true));
        }
        ...
        {
            ["args"] = args,
            ["skill"] = "pilotdeck-rag:glm-web-search",
        }, JsonWriteOptions);
...
public static partial class NativeAgentRuntime
{
    ...
    {
        var canonicalName = AgentToolNameCanonicalizer.Canonical(call.Name);
        if (!canonicalName.StartsWith("pilotdeck-rag:", StringComparison.OrdinalIgnoreCase))
        {
