...
public sealed class AgentToolExecutor
{
    ...
    public async Task<AgentToolResult> ExecuteAsync(AgentToolCall rawCall, AgentToolExecutionContext context)
    {
        ...
        try
        {
            return call.Name switch
            {
                ...
                "WebFetch" => Ok(call, "WebFetch is disabled. Use Skill with pilotdeck-rag:rag-research for source-grounded web evidence."),
        ...
        CancellationToken cancellationToken)
    {
        ...
        _ = await GitOutputAsync(
            ...
            30_000,
            cancellationToken);
        var parent = Path.Combine(Path.GetTempPath(), "pilotdeck-worktrees");
