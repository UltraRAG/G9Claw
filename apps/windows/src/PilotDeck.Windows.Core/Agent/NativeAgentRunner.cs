...
public sealed class NativeAgentRunner
{
    ...
    {
        if (request.ProviderConfig.Provider != SessionProvider.PilotDeck)
        {
    ...
    private static AgentInteractivePayload BuildInteractivePayload(string inputJson)
    {
        try
        {
            ...
            if (root.TryGetProperty("questions", out var questionValues) && questionValues.ValueKind == JsonValueKind.Array)
            {
                ...
                foreach (var questionValue in questionValues.EnumerateArray())
                {
                    ...
                    questions.Add(new AgentQuestion(
                        ...
                        MultiSelectValue(questionValue)));
                }
            ...
            return new AgentInteractivePayload([new AgentQuestion(header, question, QuestionOptions(root), MultiSelectValue(root))]);
        }
        catch
        {
    ...
    private static List<AgentQuestionOption> QuestionOptions(JsonElement root)
    {
        ...
        if (!TryGetOptionArray(root, out var values))
        {
            return options;
        }
        ...
        foreach (var option in values.EnumerateArray())
        {
            ...
            else
            {
                options.Add(new AgentQuestionOption(
                    ...
                    option.ToString(),
                    StringValue(option, "description") ?? StringValue(option, "detail")));
            }
        }

        return options
            .Where(option => !string.IsNullOrWhiteSpace(option.Label))
            .GroupBy(option => option.Label.Trim(), StringComparer.Ordinal)
            .Select(group => group.First() with { Label = group.Key })
            .ToList();
    }

    private static bool TryGetOptionArray(JsonElement root, out JsonElement values)
    {
        foreach (var key in new[] { "options", "choices", "items" })
        {
            if (root.TryGetProperty(key, out values) && values.ValueKind == JsonValueKind.Array)
            {
                return true;
            }
        }
    ...
    private static bool MultiSelectValue(JsonElement root) =>
        BoolValue(root, "multiSelect") ??
        BoolValue(root, "multi_select") ??
        BoolValue(root, "multiple") ??
        BoolValue(root, "allowMultiple") ??
        BoolValue(root, "allow_multiple") ??
        false;
