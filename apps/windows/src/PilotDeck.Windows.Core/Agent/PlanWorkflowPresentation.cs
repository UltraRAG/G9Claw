...
public static class PlanExecutionTransitionPolicy
{
    public static ChatRunMode ComposerModeAfterExitPlan(string? requestedMode, ChatRunMode currentMode) =>
        string.Equals(requestedMode, "plan", StringComparison.OrdinalIgnoreCase)
            ? currentMode
            : ChatRunMode.Agent;

    public static bool ShouldForceScrollToBottomOnPermissionChange(
        ChatScrollSnapshot snapshot,
        bool isResolvingPermission,
        bool autoScrollToBottom)
    {
        ...
        return autoScrollToBottom && snapshot.StickToBottom;
    }
}
