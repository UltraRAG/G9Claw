...
public static class SessionProviderExtensions
{
    ...
    {
        SessionProvider.PilotDeck => "pilotdeck",
    ...
    public static string DisplayName(this SessionProvider provider) =>
        provider == SessionProvider.PilotDeck ? "PilotDeck" : provider.ToString();

    public static bool IsNativeAvailable(this SessionProvider provider) =>
        provider == SessionProvider.PilotDeck;
}
...
public static class ComposerPermissionModeCatalog
{
    ...
    [
        ...
        new(ComposerPermissionMode.BypassPermissions, "bypassPermissions", "\u5141\u8bb8\u6240\u6709\u6743\u9650", "shield.lefthalf.filled", "Allow all trusted tool actions for this run."),
    ];
...
{
    public static ContextBudgetSnapshot FromBudget(TokenBudget? budget, string? compactStage = null, int compactCount = 0, bool chinese = false)
    {
        ...
        var detail = chinese
            ? $"{LevelLabel(level, chinese: true)}: {Math.Max(0, budget.Used):N0} / {budget.Total:N0} tokens ({percent}%)"
            : $"{LevelLabel(level)}: {Math.Max(0, budget.Used):N0} / {budget.Total:N0} tokens ({percent}%)";
        return new ContextBudgetSnapshot(
    ...
    public static string LevelLabel(ContextBudgetLevel? level, bool chinese) => chinese
        ? level switch
        {
            ContextBudgetLevel.Attention => "\u4e0a\u4e0b\u6587\u9700\u5173\u6ce8",
            ContextBudgetLevel.Warning => "\u4e0a\u4e0b\u6587\u8b66\u544a",
            ContextBudgetLevel.Compacting => "\u4e0a\u4e0b\u6587\u538b\u7f29\u4e2d",
            ContextBudgetLevel.Recovering => "\u4e0a\u4e0b\u6587\u6062\u590d\u4e2d",
            ContextBudgetLevel.Normal => "\u4e0a\u4e0b\u6587\u6b63\u5e38",
            _ => "\u672a\u77e5",
        }
        : LevelLabel(level);
}
    ...
    Dictionary<string, string> Headers)
{
    ...
    {
        var id = NativeSettingsIds.Normalize(Id, index == 0 ? "pilotdeck" : $"provider{index + 1}");
        return this with
        {
            Id = id,
            BaseUrl = BaseUrl?.Trim() ?? "",
            SecretAccount = string.IsNullOrWhiteSpace(SecretAccount) ? $"pilotdeck-provider-{id}" : SecretAccount.Trim(),
    ...
    int ContextWindow)
{
    ...
    public NativeModelEntry Normalize(IReadOnlyList<NativeProviderEntry> providers, int fallbackContextWindow, int index = 0)
    {
        ...
        var providerId = string.IsNullOrWhiteSpace(ProviderId) ? providers.FirstOrDefault()?.Id ?? "pilotdeck" : ProviderId.Trim();
    ...
    string HttpsProxy = "")
{
    public static NativeRuntimeSettings Defaults(string homePath)
    {
        ...
        return new NativeRuntimeSettings(
            ...
            120_000,
            160_000,
            System.IO.Path.Combine(dataRoot, "pilotdeck.db"),
            "");
    }
    ...
    int GlmWebSearchDefaultTopK)
{
    public static NativeRagSettings Defaults => new(
        ...
        false,
        "",
        "pilotdeck-rag-local-knowledge",
        "",
        "",
        5,
        "",
        "pilotdeck-rag-glm-web-search",
        5);
    ...
    int ProxyPort)
{
    ...
    public NativeGatewaySettings Normalize(string homePath) => this with
    {
        Home = string.IsNullOrWhiteSpace(Home)
            ? System.IO.Path.Combine(homePath, ".pd")
