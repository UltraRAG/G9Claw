...
public static class SkillRuntimeEnvironment
{
        ...
        string? pluginRoot = null)
    {
        ...
        environment.Remove("PILOTDECK_PLUGIN_ROOT");
        var resolvedPluginRoot = pluginRoot ?? PluginRoot();
        if (!string.IsNullOrWhiteSpace(resolvedPluginRoot))
        {
            environment["PILOTDECK_PLUGIN_ROOT"] = resolvedPluginRoot;
        }
        ...
        {
            environment[$"PILOTDECK_{key}"] = value;
        }
    ...
    {
        return PluginRoot(
            Environment.GetEnvironmentVariable("PILOTDECK_PLUGIN_ROOT"),
            Environment.GetEnvironmentVariable("PILOTDECK_REPO_ROOT"),
    ...
    private static IEnumerable<string> CandidatePluginRoots(string start)
    {
        ...
        {
            yield return Path.Combine(directory.FullName, "Assets", "pilotdeck-rag-plugin");
            yield return Path.Combine(directory.FullName, "apps", "macos-native", "PilotDeck", "Assets", "pilotdeck-rag-plugin");
