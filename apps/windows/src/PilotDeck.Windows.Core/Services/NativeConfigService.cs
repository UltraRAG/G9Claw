...
public static class NativeConfigService
{
    public static string DefaultConfigPath(string? homePath = null)
    {
        var home = string.IsNullOrWhiteSpace(homePath)
            ...
            : homePath;
        return Path.Combine(home, ".pd", "config.yaml");
    }

    public static string EnsureDefaultConfigMigrated(string? homePath = null)
    {
        var current = DefaultConfigPath(homePath);
        if (File.Exists(current))
        {
            return current;
        }

        var legacy = LegacyConfigPath(homePath);
    ...
    {
        var file = path ?? EnsureDefaultConfigMigrated();
    ...
    private static string LegacyConfigPath(string? homePath = null)
    {
        var home = string.IsNullOrWhiteSpace(homePath)
            ? Environment.GetFolderPath(Environment.SpecialFolder.UserProfile)
            : homePath;
        return Path.Combine(home, string.Concat(".", "g", "9", "claw"), "config.yaml");
    }
    ...
    public static ProviderConfig? ProviderConfigFor(string entryId, Dictionary<string, string> values)
    {
        ...
        var secretAccount = providerId is "pilotdeck"
            ? ProviderConfig.Empty.SecretAccount
            : $"pilotdeck-provider-{providerId}";
    ...
    public static string ProviderId(string entryId, Dictionary<string, string> values) =>
        Values.TryGetNonBlank(values, $"models.entries.{entryId}.provider") ?? "pilotdeck";
