...
{
    public const string Source = "pilotdeck";

    public static string Filename(DateTimeOffset date)
    {
        var utc = date.ToUniversalTime();
        return $"pilotdeck-permissions-{utc.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture)}.json";
    }
}

public enum PilotDeckConfigSection
{
    Runtime,
    Models,
    AlwaysOn,
...
public static class PilotDeckConfigSectionExtensions
{
    public static string Id(this PilotDeckConfigSection section) => section switch
    {
        PilotDeckConfigSection.AlwaysOn => "alwaysOn",
        PilotDeckConfigSection.Rag => "rag",
        _ => section.ToString()[..1].ToLowerInvariant() + section.ToString()[1..],
    };

    public static string Label(this PilotDeckConfigSection section) => section switch
    {
        PilotDeckConfigSection.Runtime => "Runtime",
        PilotDeckConfigSection.Models => "Models",
        PilotDeckConfigSection.AlwaysOn => "Always-On",
        PilotDeckConfigSection.Memory => "Memory",
        PilotDeckConfigSection.Rag => "RAG",
        PilotDeckConfigSection.Router => "Router",
        PilotDeckConfigSection.Gateway => "Gateway",
        PilotDeckConfigSection.Raw => "Raw YAML",
        _ => section.ToString(),
    };
}
