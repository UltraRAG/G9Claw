...
public sealed record MarkdownBlockPresentation(
    ...
    IReadOnlyList<bool?>? ListTaskStates = null,
...
public static class MarkdownPresentation
{
    ...
    private static void AddBlock(Block block, List<MarkdownBlockPresentation> blocks)
    {
        switch (block)
        {
            ...
            case QuoteBlock quote:
                ...
                break;
            case ListBlock list:
                var (items, taskStates) = ListItems(list);
                blocks.Add(new MarkdownBlockPresentation(
                    MarkdownBlockKind.List,
                    [],
                    ListItems: items,
                    ListTaskStates: taskStates,
            ...
            case ThematicBreakBlock:
                blocks.Add(new MarkdownBlockPresentation(MarkdownBlockKind.ThematicBreak, []));
                break;
    ...
    private static (IReadOnlyList<IReadOnlyList<MarkdownInlinePresentation>> Items, IReadOnlyList<bool?> TaskStates) ListItems(ListBlock list)
    {
        var items = new List<IReadOnlyList<MarkdownInlinePresentation>>();
        var taskStates = new List<bool?>();
        foreach (var item in list.OfType<ListItemBlock>())
        {
            var inlines = ListItemInlines(item).ToList();
            taskStates.Add(ExtractTaskState(inlines));
            items.Add(MergeAdjacentText(inlines));
        }

        return (items, taskStates);
    }

    private static IReadOnlyList<MarkdownInlinePresentation> ListItemInlines(ListItemBlock item)
    {
        ...
        foreach (var child in item)
        {
            if (child is ParagraphBlock paragraph)
            {
                ...
                result.AddRange(InlineContent(paragraph.Inline));
                continue;
            }

            var text = LeafText(child);
            ...
            result.Add(new MarkdownInlinePresentation(MarkdownInlineKind.Text, text));
        }

        return result;
    }

    private static bool? ExtractTaskState(List<MarkdownInlinePresentation> inlines)
    {
        if (inlines.Count == 0 || inlines[0].Kind != MarkdownInlineKind.Text)
        {
            return null;
        }

        var text = inlines[0].Text;
        ...
        inlines[0] = inlines[0] with { Text = text[4..] };
        return isChecked;
    }
    ...
    private static string LeafText(MarkdownObject block)
    {
        ...
        foreach (var inline in block.Descendants().OfType<Inline>())
        {
            switch (inline)
            {
                case LiteralInline literal:
