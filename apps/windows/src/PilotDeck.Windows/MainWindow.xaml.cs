...
public sealed partial class MainWindow : Window
{
    ...
    private readonly WorkspaceSessionStore _workspaceSessionStore;
    ...
    private EditorDirtyState? _previewDirtyState;
    private bool _suppressPreviewTextChanged;
    private bool _isFileEditorExpanded;
    private bool _isSidebarVisible = true;
    private bool _isSidebarAnimating;
    private bool _forceChatScrollToBottomOnce;
    ...
    public MainWindow()
    {
        ...
        _workspaceSessionStore = new WorkspaceSessionStore();
    ...
    private bool IsPointInCustomDragRegion(IntPtr lParam)
    {
        if (_hwnd == IntPtr.Zero ||
            ...
            ProjectCreationOverlayRoot.Visibility == Visibility.Visible ||
        ...
        {
            return !IsPointInHeaderCaptionReserve(point) &&
                   !IsInElement(point, OpenSidebarButton) &&
                   !IsInElement(point, ToolTabsScroll) &&
                   !IsInElement(point, WindowControlsRoot);
        }
    ...
    private static FrameworkElement SpinnerHost(
        double hostSize = InteractiveCardMetrics.SpinnerHostSize,
        double ringSize = InteractiveCardMetrics.SpinnerRingSize,
        bool active = true,
        ...
        Visibility visibility = Visibility.Visible)
    {
        var host = new Grid
        {
            ...
            Clip = new RectangleGeometry { Rect = new global::Windows.Foundation.Rect(0, 0, hostSize, hostSize) },
        };
    ...
    private bool IsPointInHeaderCaptionReserve(global::Windows.Foundation.Point rootPoint)
    {
        ...
        try
        {
            var origin = HeaderRoot.TransformToVisual(RootGrid).TransformPoint(new global::Windows.Foundation.Point(0, 0));
            var localX = rootPoint.X - origin.X;
            var captionInset = 0d;
            return new HeaderLayoutMetrics(HeaderRoot.ActualWidth, captionInset).IsInCaptionButtonReserve(localX);
        }
        catch
        {
            return false;
    ...
    private async Task LoadSettingsAsync()
    {
        ...
        _workspaceService = new WorkspaceService(State.Settings.WorkspacesRoot);
        if (_workspaceSessionStore.LoadSnapshot() is { } workspaceSnapshot)
        {
            _workspaceSessionStore.RestoreInto(State, workspaceSnapshot);
        }

        State.RoutingUsage.Clear();
        State.RoutingUsage.AddRange(_workspaceSessionStore.LoadRoutingUsage());
    ...
    private void ApplyUiSettings()
    {
        ...
        {
            SidebarColumn.Width = new GridLength(_isSidebarVisible ? _uiSettings.SidebarWidth : 0);
        }
    ...
    private void AnimateSidebarVisibility(bool visible)
    {
        _sidebarAnimationTimer?.Stop();
        _sidebarAnimationTimer = null;

        var startWidth = SidebarColumn.Width.IsAbsolute ? SidebarColumn.Width.Value : (_isSidebarVisible ? _uiSettings.SidebarWidth : 0);
        var targetWidth = visible ? _uiSettings.SidebarWidth : 0;
        ...
        OpenSidebarButton.Visibility = visible ? Visibility.Collapsed : Visibility.Visible;
        ScheduleApplyHeaderMetrics();

        var watch = Stopwatch.StartNew();
        var timer = new DispatcherTimer { Interval = TimeSpan.FromMilliseconds(16) };
        _sidebarAnimationTimer = timer;
        timer.Tick += (_, _) =>
        {
            var progress = Math.Clamp(watch.Elapsed.TotalMilliseconds / SidebarAnimationMetrics.DurationMs, 0, 1);
            var width = SidebarAnimationMetrics.WidthAt(startWidth, targetWidth, progress);
            SidebarColumn.Width = new GridLength(Math.Max(0, width));
            if (progress < 1) return;

            timer.Stop();
            if (ReferenceEquals(_sidebarAnimationTimer, timer))
            {
                _sidebarAnimationTimer = null;
            }
            ...
            SidebarRoot.Visibility = visible ? Visibility.Visible : Visibility.Collapsed;
            SidebarDivider.Visibility = visible ? Visibility.Visible : Visibility.Collapsed;
            OpenSidebarButton.Visibility = visible ? Visibility.Collapsed : Visibility.Visible;
            ScheduleApplyHeaderMetrics();
        };
        timer.Start();
    ...
    private void RestoreChatScrollState(ScrollViewer scroll)
    {
        DispatcherQueue.TryEnqueue(() =>
        {
            var target = ChatScrollPresenter.TargetOffset(
                ...
                _forceChatScrollToBottomOnce);
            _forceChatScrollToBottomOnce = false;
    ...
    private void ForceChatScrollToBottomOnNextRender()
    {
        _chatStickToBottom = true;
        _forceChatScrollToBottomOnce = true;
    }

    private void BeginChatScrollTransaction(ChatScrollMutation mutation)
    {
        var transaction = ChatScrollTransactionPolicy.ForMutation(mutation, _chatStickToBottom);
        if (transaction.ForceBottom)
        {
            ForceChatScrollToBottomOnNextRender();
        }
    }

    private void RenderChatSurface(bool refreshSidebar = false)
    {
        if (refreshSidebar)
        {
            RenderSidebar();
    ...
    private FrameworkElement SessionDot(SessionState state)
    {
        ...
        {
            var spinner = SpinnerHost(16, 12, true);
            spinner.VerticalAlignment = VerticalAlignment.Top;
            spinner.Margin = new Thickness(0, 2, 0, 0);
            return spinner;
        }
    ...
    {
        ClampActiveTabToSelectedProject();
    ...
    {
        return AppTabCatalog.ToolSwitcherTabsForProject(State.SelectedProject);
    }
    ...
    private void RenderContent()
    {
        ...
        var shouldRefreshHeader = ClampActiveTabToSelectedProject();
    ...
    private bool ClampActiveTabToSelectedProject()
    {
        var clamped = AppTabCatalog.ClampToolSwitcherTab(State.ActiveTab, State.SelectedProject);
        if (clamped == State.ActiveTab && State.ActiveTab != AppTab.Preview)
        {
            return false;
        }
    ...
    private FrameworkElement ChatPage()
    {
        ...
        if (!composerRunningStatus.ShouldRender && (_isAgentSubmitting || _isAgentRunning))
        {
            composerRunningStatus = new ComposerRunningStatusPresentation(
                true,
                _isAgentSubmitting
                    ? (IsChineseUi() ? "\u6b63\u5728\u51c6\u5907" : "Preparing")
                    : (IsChineseUi() ? "\u6b63\u5728\u751f\u6210" : "Working"),
                true,
                "",
                null);
        }
        var inlinePermissionRequests = InlinePendingPermissions();
        var inlinePermissionReserve = inlinePermissionRequests.Count == 0
            ? 66
            : inlinePermissionRequests.Any(request => request.Kind is PermissionRequestKind.ExitPlanMode or PermissionRequestKind.DestructivePlanApproval)
                ? PlanConfirmationCardMetrics.MaxPlanHeightForViewport(RootGrid.ActualHeight > 0 ? RootGrid.ActualHeight : V2LayoutMetrics.DefaultWindowHeight) + 228
                : inlinePermissionRequests.Any(request => request.Kind is PermissionRequestKind.AskUserQuestion)
                    ? 500
                    : 214;
        var footerReserve = V2LayoutMetrics.ComposerMinHeight +
            (composerRunningStatus.ShouldRender ? 34 : 0) +
            inlinePermissionReserve;
        ...
        if (!hasMessages)
        {
            ...
            showGeneralProjectEntry = GeneralProjectEntryPresentation.ShouldRender(
                State.SelectedProject,
                State.ActiveTab,
                _uiSettings.SidebarSection,
                hasMessages,
        ...
        var attachFiles = new MenuFlyoutItem { Text = T("chat.composer.attachFiles") };
        attachFiles.Click += async (_, _) => await AttachComposerFilesAsync();
        attachmentFlyout.Items.Add(attachFiles);
        var attachFolder = new MenuFlyoutItem { Text = T("chat.composer.attachFolder") };
        ...
        var modeButton = ComposerPillButton(
            ...
            ComposerRunModeLabel(State.ComposerRunMode));
        ...
        var permissionButton = ComposerPillButton(
            ...
            ComposerPermissionModeLabel(State.ComposerPermissionMode));
    ...
    private FrameworkElement ComposerRunningStatusRow(ComposerRunningStatusPresentation presentation)
    {
        ...
        row.Children.Add(SpinnerHost(16, 12, presentation.ShouldShimmer));
    ...
    private FrameworkElement GenericPermissionCard(PermissionRequest request)
    {
        ...
        actions.ColumnDefinitions.Add(new ColumnDefinition { Width = new GridLength(1, GridUnitType.Star) });
        actions.ColumnDefinitions.Add(new ColumnDefinition { Width = new GridLength(1, GridUnitType.Star) });
        var deny = PermissionActionButton(
            isChinese ? "\u62d2\u7edd" : "Deny",
            Brush("V2RedBrush"),
            Transparent,
        ...
        var allowOnce = PermissionActionButton(
        ...
        allowOnce.HorizontalAlignment = HorizontalAlignment.Stretch;
        Grid.SetColumn(allowOnce, 1);
        actions.Children.Add(allowOnce);
        var alwaysAllow = PermissionActionButton(
        ...
        alwaysAllow.HorizontalAlignment = HorizontalAlignment.Stretch;
        Grid.SetColumn(alwaysAllow, 2);
        actions.Children.Add(alwaysAllow);
    ...
    private FrameworkElement AskUserQuestionPanel(PermissionRequest request)
    {
        ...
        {
            options.Children.Add(AskQuestionOpenAnswerNotice());
        }
    ...
    private Button AskQuestionOptionButton(PermissionRequest request, AgentQuestion question, AgentQuestionOption option, int index)
    {
        ...
        content.Children.Add(new Border
        {
            Width = 20,
            Height = 20,
            CornerRadius = new CornerRadius(question.MultiSelect ? 4 : 5),
            BorderThickness = question.MultiSelect ? new Thickness(1) : new Thickness(0),
            ...
            Child = question.MultiSelect
                ? selected ? Icon("Check", 12, Brush("V2InverseForegroundBrush")) : null
                : new TextBlock
                {
                    Text = $"{index + 1}",
                    FontSize = 11,
    ...
    private FrameworkElement AskQuestionOpenAnswerNotice()
    {
        var isChinese = IsChineseUi();
        return new TextBlock
        {
            Text = isChinese ? "\u8fd9\u662f\u5f00\u653e\u95ee\u9898\uff0c\u8bf7\u586b\u5199\u7b80\u77ed\u7b54\u6848\u3002" : "This is an open question. Type a short answer.",
            FontSize = 12,
            Foreground = Brush("V2MutedForegroundBrush"),
            TextWrapping = TextWrapping.Wrap,
        };
    ...
    private JsonNode? AskQuestionAnswer(Guid requestId, AgentQuestion question)
    {
        var values = AskQuestionSelection(requestId, question)
        ...
        return values.Count == 0 ? null : JsonValue.Create(values[0].Trim());
    }
    ...
    private FrameworkElement ExitPlanModePermissionCard(PermissionRequest request)
    {
        ...
        var planMaxHeight = PlanConfirmationCardMetrics.MaxPlanHeightForViewport(RootGrid.ActualHeight > 0 ? RootGrid.ActualHeight : V2LayoutMetrics.DefaultWindowHeight);
            ...
            Child = new Grid
            {
                Children =
                {
                    new ScrollViewer
                    {
                        ...
                        MaxHeight = planMaxHeight,
                        VerticalScrollBarVisibility = ScrollBarVisibility.Auto,
                        Padding = new Thickness(14, PlanConfirmationCardMetrics.PlanBadgeTopPadding, 14, 14),
    ...
    {
        var nextMode = PlanExecutionTransitionPolicy.ComposerModeAfterExitPlan(mode, State.ComposerRunMode);
        if (nextMode != State.ComposerRunMode)
        {
            State.ComposerRunMode = nextMode;
            BeginChatScrollTransaction(ChatScrollMutation.PlanExecuted);
            RenderChatSurface();
        }
        ...
        var button = new Button
        {
            ...
            Content = new Grid
            {
                ...
                Children =
                {
                    ...
                    SpinnerHost(16, 12, presentation.ShouldShimmer, visibility: presentation.ShouldShimmer ? Visibility.Visible : Visibility.Collapsed),
    ...
    private string ComposerRunModeLabel(ChatRunMode mode) => mode switch
    {
        ChatRunMode.Plan => T("chat.composer.runMode.plan"),
        _ => T("chat.composer.runMode.agent"),
    };

    private string ComposerPermissionModeLabel(ComposerPermissionMode mode) => mode switch
    {
        ComposerPermissionMode.BypassPermissions => T("chat.composer.bypassPermissions"),
        _ => T("chat.composer.defaultPermissions"),
    };
    ...
    private MenuFlyout ComposerRunModeFlyout()
    {
        ...
        foreach (var descriptor in ChatRunModeCatalog.All)
        {
            ...
            {
                Text = ComposerRunModeLabel(descriptor.Mode),
            };
            item.Click += (_, _) =>
            {
                State.ComposerRunMode = descriptor.Mode;
                BeginChatScrollTransaction(ChatScrollMutation.LocalComposerChange);
                RenderContent();
                FocusComposerSoon();
            };
    ...
    private MenuFlyout ComposerPermissionFlyout()
    {
        ...
        foreach (var descriptor in ComposerPermissionModeCatalog.All)
        {
            var item = new MenuFlyoutItem
            {
                Text = ComposerPermissionModeLabel(descriptor.Mode),
            };
            item.Click += (_, _) =>
            {
    ...
    private Button ComposerSendButton()
    {
        ...
        FrameworkElement content = _isAgentSubmitting
            ? SpinnerHost(18, 15, true, foreground)
            : _isAgentRunning
    ...
    private bool ComposerCanSubmit() =>
        ComposerEligibility().CanSubmit;

    private ComposerSendEligibility ComposerEligibility() =>
        ComposerSendEligibilityPolicy.Evaluate(
    ...
    private Button ContextGaugeButton()
    {
        ...
        var isChinese = IsChineseUi();
        var snapshot = ContextBudgetPresenter.FromBudget(budget, _lastContextStage, _contextCompactCount, isChinese);
        ...
        button.Flyout = new Flyout
        {
            ...
            Content = ContextGaugePopover(snapshot, foreground, isChinese),
        };
        ToolTipService.SetToolTip(button, snapshot.Detail);
        return button;
    }

    private FrameworkElement ContextGaugePopover(ContextBudgetSnapshot snapshot, Brush tone, bool isChinese)
    {
        ...
        var headerPills = new StackPanel
        {
            Orientation = Orientation.Horizontal,
            Spacing = 6,
        };
        if (snapshot.Level is not null)
        {
            headerPills.Children.Add(ContextPill(ContextBudgetLevelLabel(snapshot.Level), tone));
        }

        if (snapshot.Percent is not null)
        {
            headerPills.Children.Add(ContextPill($"{snapshot.Percent}%", tone));
        }

        if (headerPills.Children.Count > 0)
        {
            Grid.SetColumn(headerPills, 1);
            header.Children.Add(headerPills);
        }
        ...
        if (!string.IsNullOrWhiteSpace(snapshot.CompactStage))
        {
            panel.Children.Add(new TextBlock
            {
                Text = string.Format(T("chat.context.recentStage"), snapshot.CompactStage),
                FontSize = 12,
                Foreground = Brush("V2MutedForegroundBrush"),
                TextWrapping = TextWrapping.Wrap,
            });
        ...
        if (snapshot.CompactCount > 0)
        {
            panel.Children.Add(new TextBlock
            {
                Text = string.Format(T("chat.context.compactionCount"), snapshot.CompactCount),
                FontSize = 12,
                Foreground = Brush("V2MutedForegroundBrush"),
                TextWrapping = TextWrapping.Wrap,
            });
    ...
    private string ContextBudgetLevelLabel(ContextBudgetLevel? level) => level switch
    {
        ContextBudgetLevel.Attention => T("chat.context.level.attention"),
        ContextBudgetLevel.Warning => T("chat.context.level.warning"),
        ContextBudgetLevel.Compacting => T("chat.context.level.compacting"),
        ContextBudgetLevel.Recovering => T("chat.context.level.recovering"),
        ContextBudgetLevel.Normal => T("chat.context.level.normal"),
        _ => T("common.failed"),
    };

    private Border ContextPill(string text, Brush tone) => new()
    {
    ...
    private FrameworkElement MarkdownBlockElement(MarkdownBlockPresentation block)
    {
        ...
        {
            MarkdownBlockKind.Heading => MarkdownHeading(block),
            ...
            MarkdownBlockKind.ThematicBreak => MarkdownDivider(),
            _ => MarkdownTextBlock(block.Inlines, 15, Microsoft.UI.Text.FontWeights.Normal),
        };
    }

    private FrameworkElement MarkdownDivider() => new Border
    {
        Height = 1,
    ...
    private FrameworkElement MarkdownHeading(MarkdownBlockPresentation block)
    {
        var heading = MarkdownTextBlock(
            block.Inlines,
            block.HeadingLevel switch
            {
                1 => 22,
                2 => 19,
                3 => 17,
        ...
        heading.Margin = new Thickness(0, block.HeadingLevel <= 2 ? 4 : 2, 0, 0);
        return heading;
    }
    ...
    private FrameworkElement MarkdownList(MarkdownBlockPresentation block)
    {
        ...
        {
            var taskState = block.ListTaskStates is { Count: > 0 } states && i < states.Count
                ? states[i]
                : null;
            var row = new Grid { ColumnSpacing = 8 };
            row.ColumnDefinitions.Add(new ColumnDefinition { Width = new GridLength(block.Ordered || taskState.HasValue ? 22 : 12) });
            row.ColumnDefinitions.Add(new ColumnDefinition { Width = new GridLength(1, GridUnitType.Star) });
            FrameworkElement marker = taskState.HasValue
                ? new Border
                {
                    Width = 16,
                    Height = 16,
                    ...
                    BorderThickness = new Thickness(1),
                    BorderBrush = taskState.Value ? Brush("V2BlueBrush") : Brush("V2BorderBrush"),
                    Background = taskState.Value ? Brush("V2BlueBrush") : Brush("V2CardBrush"),
                    Child = taskState.Value ? Icon("Check", 11, Brush("V2InverseForegroundBrush")) : null,
                    HorizontalAlignment = HorizontalAlignment.Right,
                    VerticalAlignment = VerticalAlignment.Top,
            ...
            var content = MarkdownTextBlock(items[i], 15, Microsoft.UI.Text.FontWeights.Normal);
    ...
    private FrameworkElement MarkdownQuote(MarkdownBlockPresentation block)
    {
        ...
        var text = MarkdownTextBlock(block.Inlines, 15, Microsoft.UI.Text.FontWeights.Normal);
        ...
        for (var r = 0; r < rows.Count; r++)
        {
            ...
            for (var c = 0; c < rows[r].Count; c++)
            {
                var cell = new Border
                {
                    ...
                    Child = MarkdownTextBlock([rows[r][c]], table.HasHeader && r == 0 ? 13.5 : 13, table.HasHeader && r == 0
                        ? Microsoft.UI.Text.FontWeights.SemiBold
    ...
    private FrameworkElement FileRow(WorkspaceFile file)
    {
        ...
        var row = new Button
        {
            Height = FileTreeRowMetrics.RowHeight,
            Background = isSelected ? Brush("V2SelectedBrush") : Transparent,
            BorderBrush = Transparent,
            HorizontalAlignment = HorizontalAlignment.Stretch,
            HorizontalContentAlignment = HorizontalAlignment.Stretch,
            Padding = new Thickness(FileTreeRowMetrics.LeadingPadding(file.Depth), 0, FileTreeRowMetrics.BaseHorizontalPadding, 0),
            CornerRadius = new CornerRadius(6),
        ...
        grid.ColumnDefinitions.Add(new ColumnDefinition { Width = new GridLength(12) });
        grid.ColumnDefinitions.Add(new ColumnDefinition { Width = new GridLength(FileTreeRowMetrics.IconColumnWidth) });
        grid.ColumnDefinitions.Add(new ColumnDefinition { Width = new GridLength(1, GridUnitType.Star) });
        grid.ColumnDefinitions.Add(new ColumnDefinition { Width = new GridLength(FileTreeRowMetrics.ActionColumnWidth) });
    ...
    private FrameworkElement FileInlineEditRow(FileInlineEditState edit)
    {
        ...
        {
            Height = FileTreeRowMetrics.RowHeight,
            Padding = new Thickness(FileTreeRowMetrics.LeadingPadding(edit.Depth), 0, FileTreeRowMetrics.BaseHorizontalPadding, 0),
            ColumnSpacing = 6,
            HorizontalAlignment = HorizontalAlignment.Stretch,
            ColumnDefinitions =
            {
                new ColumnDefinition { Width = new GridLength(12) },
                new ColumnDefinition { Width = new GridLength(FileTreeRowMetrics.IconColumnWidth) },
                new ColumnDefinition { Width = new GridLength(1, GridUnitType.Star) },
            },
        };
    ...
    private FrameworkElement FilePreviewPane()
    {
        ...
        try
        {
            ...
            if (preview.Text is not null)
            {
                ...
                {
                    ApplyPreviewDirtyState(preview, EditorDirtyChangeKind.Revert, preview.Text);
                    RenderContent();
                };
            ...
            FrameworkElement content = preview.Kind switch
            {
                ...
                WorkspacePreviewKind.Text or WorkspacePreviewKind.Markdown or WorkspacePreviewKind.Html => TextPreview(preview, UpdateDirtyChrome, languageAlias),
    ...
    private void EnsurePreviewDraft(WorkspacePreview preview)
    {
        if (preview.Text is null) return;
        ApplyPreviewDirtyState(preview, EditorDirtyChangeKind.OpenFile, preview.Text);
    }

    private void ApplyPreviewDirtyState(WorkspacePreview preview, EditorDirtyChangeKind change, string text)
    {
        var path = preview.RelativePath;
        _previewDirtyState = _previewDirtyState is null
            ? EditorDirtyState.ForFile(path, text)
            : _previewDirtyState.Apply(change, path, text);
        _previewDraftText = _previewDirtyState.DraftText;
    }

    private void ResetPreviewDraft()
    {
        _previewDraftText = null;
        _previewDirtyState = null;
    }
    ...
    private bool PreviewHasUnsavedChanges(WorkspacePreview preview)
    {
        ...
        EnsurePreviewDraft(preview);
        return _previewDirtyState?.IsDirty == true;
    }

    private static string NormalizedEditorText(string text) =>
        text.Replace("\r\n", "\n").Replace('\r', '\n');
    ...
    private FrameworkElement TextPreview(WorkspacePreview preview, Action<bool>? onDirtyChanged = null, string? languageAlias = null)
    {
        EnsurePreviewDraft(preview);
        _previewDraftText ??= preview.Text ?? "";
        var root = new Grid
        ...
        var showLineNumbers = State.Settings.EditorSettings.LineNumbers;
        var showMinimap = State.Settings.EditorSettings.ShowMinimap;
        var editorFontSize = State.Settings.EditorSettings.FontSize;
        var editorLineHeight = EditableCodeEditorMetrics.LineHeight(editorFontSize);
        var currentEditorText = _previewDraftText;
        ScrollViewer? lineNumberScroller = null;
        Border? minimapHost = null;
        ScrollViewer? editorScroller = null;

        if (showLineNumbers)
        {
            root.ColumnDefinitions.Add(new ColumnDefinition { Width = new GridLength(CodeLineNumberMetrics.RulerWidth(lineCount)) });
        }
        ...
        var editor = new RichEditBox
        {
            ...
            Padding = new Thickness(12, CodeLineNumberMetrics.EditorTextInset.Height, 12, CodeLineNumberMetrics.EditorTextInset.Height),
            BorderThickness = new Thickness(0),
            Background = Brush("V2BackgroundBrush"),
            HorizontalAlignment = HorizontalAlignment.Stretch,
            VerticalAlignment = VerticalAlignment.Stretch,
        ...
        editor.Document.SetText(TextSetOptions.None, _previewDraftText);
        ApplyEditorParagraphMetrics();
        ScrollViewer.SetVerticalScrollMode(editor, ScrollMode.Enabled);
        ScrollViewer.SetHorizontalScrollMode(
            editor,
            State.Settings.EditorSettings.WordWrap ? ScrollMode.Disabled : ScrollMode.Enabled);
        ScrollViewer.SetVerticalScrollBarVisibility(editor, ScrollBarVisibility.Auto);
        ScrollViewer.SetHorizontalScrollBarVisibility(
            editor,
            State.Settings.EditorSettings.WordWrap ? ScrollBarVisibility.Disabled : ScrollBarVisibility.Auto);

        if (showLineNumbers)
        {
            lineNumberScroller = new ScrollViewer
            {
                Content = CodeLineNumberGutter(lineCount),
            ...
            root.Children.Add(lineNumberScroller);
        }

        var editorColumn = showLineNumbers ? 1 : 0;
        Grid.SetColumn(editor, editorColumn);
        root.Children.Add(editor);

        if (showMinimap)
        {
            minimapHost = CodeMinimapShell();
            minimapHost.IsHitTestVisible = CodeEditorScrollStabilityMetrics.MinimapAllowsHitTesting;
            Grid.SetColumn(minimapHost, editorColumn + 1);
            root.Children.Add(minimapHost);
        }
        ...
        void ApplyEditorParagraphMetrics()
        {
            ...
            try
            {
                _suppressPreviewTextChanged = true;
                var range = editor.Document.GetRange(0, Math.Max(0, currentEditorText.Length));
                range.ParagraphFormat.SetLineSpacing(LineSpacingRule.Exactly, EditableCodeEditorMetrics.RichEditLineSpacingPoints(editorFontSize));
            ...
            {
                // Paragraph metrics are best-effort on WinUI RichEditBox versions.
            }
            finally
            {
                _suppressPreviewTextChanged = false;
            }
        }
        ...
        void ApplySyntaxHighlighting()
        {
            ...
            if (!CodeSyntaxHighlightingService.ShouldHighlight(text, languageAlias) ||
                text.Length > EditableCodeEditorMetrics.MaxInteractiveHighlightedCharacters)
            {
                return;
            }

            var spans = CodeSyntaxHighlightingService.HighlightedSpans(text, languageAlias);
            if (!EditableCodeEditorMetrics.CanApplyInteractiveSyntaxHighlighting(text.Length, spans.Count))
            {
                return;
            }
            ...
            {
                _suppressPreviewTextChanged = true;
                var fullRange = editor.Document.GetRange(0, Math.Max(0, text.Length));
                fullRange.CharacterFormat.ForegroundColor = ColorFromHex(CodeSyntaxHighlightingService.ColorHex(CodeHighlightTokenKind.Plain, RootGrid.ActualTheme == ElementTheme.Dark));
                var offset = 0;
                foreach (var span in spans)
                {
                    ...
                    var end = Math.Min(text.Length, offset + span.Text.Length);
                    if (end > offset)
                    {
                        var range = editor.Document.GetRange(offset, end);
                        range.CharacterFormat.ForegroundColor = ColorFromHex(CodeSyntaxHighlightingService.ColorHex(span.Kind, RootGrid.ActualTheme == ElementTheme.Dark));
                    }
            ...
            {
                // Syntax coloring is best-effort; editing must remain available.
            }
            finally
            {
                _suppressPreviewTextChanged = false;
        ...
        DispatcherTimer? highlightTimer = null;
        void ScheduleSyntaxHighlighting()
        {
            highlightTimer ??= new DispatcherTimer
            {
                Interval = CodeSyntaxHighlightingService.HighlightDebounceInterval,
            };
        ...
        void OnHighlightTimerTick(object? sender, object e)
        {
            if (highlightTimer is null)
            {
                return;
            }
        ...
        Range? VisibleRange()
        {
            var totalLines = CodeLineNumberMetrics.LineCount(currentEditorText);
            return editorScroller is { ViewportHeight: > 0 } scroll
                ? EditableCodeEditorMetrics.VisibleLineRange(
                    scroll.VerticalOffset,
                    scroll.ViewportHeight,
                    editorFontSize,
                    totalLines)
        ...
        void SyncEditorChrome()
        {
            if (editorScroller is not null && lineNumberScroller is not null)
            {
                lineNumberScroller.ChangeView(null, editorScroller.VerticalOffset, null, true);
            }

            if (minimapHost is not null)
            {
                UpdateCodeMinimap(minimapHost, CodeMinimapModel.FromText(currentEditorText, VisibleRange()));
            }
        }

        void RefreshEditorChrome()
        {
            var nextLineCount = CodeLineNumberMetrics.LineCount(currentEditorText);
            if (lineNumberScroller is not null && nextLineCount != lineCount)
            {
                lineCount = nextLineCount;
                lineNumberScroller.Content = CodeLineNumberGutter(lineCount);
            }
        ...
        {
            if (_suppressPreviewTextChanged)
            {
                return;
            }

            _previewDraftText = EditorText();
            currentEditorText = _previewDraftText;
            ApplyEditorParagraphMetrics();
            ApplyPreviewDirtyState(preview, EditorDirtyChangeKind.UserEdit, _previewDraftText);
        ...
        editor.Loaded += (_, _) =>
        {
            editorScroller ??= FindVisualChild<ScrollViewer>(editor);
            if (editorScroller is not null)
            {
                editorScroller.ViewChanged += (_, _) => SyncEditorChrome();
            }

            RefreshEditorChrome();
            ScheduleSyntaxHighlighting();
        };
        editor.SizeChanged += (_, _) => SyncEditorChrome();

        if (minimapHost is not null)
        {
            void SeekMinimap(PointerRoutedEventArgs args)
            {
                if (editorScroller is null || minimapHost.ActualHeight <= 0)
                {
                    return;
                }

                var point = args.GetCurrentPoint(minimapHost).Position;
                var model = CodeMinimapModel.FromText(currentEditorText, VisibleRange());
                var line = model.LineNumberAt(point.Y, minimapHost.ActualHeight);
                editorScroller.ChangeView(
                    null,
                    EditableCodeEditorMetrics.ScrollOffsetForLine(line, editorFontSize),
                    null,
                    true);
                args.Handled = true;
            }

            minimapHost.PointerPressed += (_, args) =>
            {
                minimapHost.CapturePointer(args.Pointer);
                SeekMinimap(args);
            };
            minimapHost.PointerMoved += (_, args) =>
            {
                if (args.GetCurrentPoint(minimapHost).Properties.IsLeftButtonPressed)
                {
                    SeekMinimap(args);
                }
            };
            minimapHost.PointerReleased += (_, args) =>
            {
                minimapHost.ReleasePointerCapture(args.Pointer);
                args.Handled = true;
            };
        }
    ...
    {
        var fontSize = State.Settings.EditorSettings.FontSize;
        var lineHeight = EditableCodeEditorMetrics.LineHeight(fontSize);
        var panel = new StackPanel
        {
            Background = Brush("V2BackgroundBrush"),
            Padding = new Thickness(0, CodeLineNumberMetrics.EditorTextInset.Height, EditableCodeEditorMetrics.GutterRightPadding, CodeLineNumberMetrics.EditorTextInset.Height),
        };
    ...
    {
        var host = CodeMinimapShell();
        void Refresh() => UpdateCodeMinimap(host, model);
        host.Loaded += (_, _) => Refresh();
        host.SizeChanged += (_, _) => Refresh();
        Refresh();
        return host;
    }

    private Border CodeMinimapShell() => new()
    {
    ...
    private void UpdateCodeMinimap(Border host, CodeMinimapModel model)
    {
        var height = host.ActualHeight > 0
            ? host.ActualHeight
            : CodeMinimapFallbackHeight(model);
        host.Child = CodeMinimapCanvas(model, height);
    }

    private static double CodeMinimapFallbackHeight(CodeMinimapModel model) =>
        Math.Max(120, Math.Min(720, model.Lines.Count * 2.0));

    private Canvas CodeMinimapCanvas(CodeMinimapModel model, double height)
    {
        ...
        Canvas.SetTop(viewport, Math.Min(Math.Max(0, height - viewportHeight), model.ViewportStartFraction * height));
            ...
            switch (edit.Kind)
            {
                ...
                case FileInlineEditKind.CreateFolder:
                {
                    ...
                    else
                    {
                        ...
                        _previewDirtyState = EditorDirtyState.ForFile(_selectedFilePath, "");
                        _previewDraftText = "";
                        _isMarkdownPreviewing = false;
                        _isCodePreviewing = false;
                    }
    ...
    private async Task SavePreviewAsync()
    {
        ...
        try
        {
            ...
            if (_workspaceService.Preview(_selectedFilePath, State.SelectedProject.RootPath) is { Text: { } savedText } savedPreview)
            {
                ApplyPreviewDirtyState(savedPreview, EditorDirtyChangeKind.Save, savedText);
            }
            RenderContent();
        }
        catch (Exception ex)
    ...
    private FrameworkElement GitPage()
    {
        ...
        var page = ToolPage(T("git.title"), State.SelectedProject.RootPath, new StackPanel
        {
            ...
            Children =
            {
                ...
                ToolbarTextButton("Pull", (Action)(async () => await GitActionAsync(() => { _gitService.Pull(State.SelectedProject.RootPath, "PilotDeck", "pilotdeck@example.local"); return "Pulled current branch."; }))),
    ...
    private async Task CommitGitAsync()
    {
        ...
        await GitActionAsync(() =>
        {
            ...
            var commit = _gitService.Commit(State.SelectedProject.RootPath, message, paths, "PilotDeck", "pilotdeck@example.local");
    ...
    private FrameworkElement RoutingPage()
    {
        ...
        scroll.SizeChanged += (_, args) =>
        {
            ...
            {
                panel.Width = Math.Min(1320, Math.Max(0, args.NewSize.Width));
            }
        };
        root.Children.Add(scroll);
        return root;
    ...
    private FrameworkElement TerminalRunRow(NativeBackgroundRun run)
    {
        ...
        var exitText = run.Status == PilotDeck.Windows.Core.TaskStatus.Running ? "running" : $"exit {run.ExitCode ?? -1}";
    ...
    {
        WindowMinimizeButton.Background = Transparent;
        WindowMinimizeButton.BorderBrush = Transparent;
        WindowMinimizeButton.Content = WindowChromeIcon("Minus");
        ToolTipService.SetToolTip(WindowMinimizeButton, T("window.minimize"));
        WindowMaximizeButton.Background = Transparent;
        WindowMaximizeButton.BorderBrush = Transparent;
        WindowMaximizeButton.Content = WindowChromeIcon("Square");
        ToolTipService.SetToolTip(WindowMaximizeButton, T("window.maximize"));
        WindowCloseButton.Background = Transparent;
        WindowCloseButton.BorderBrush = Transparent;
        WindowCloseButton.Content = WindowChromeIcon("X");
        ToolTipService.SetToolTip(WindowCloseButton, T("window.close"));
    ...
    {
        var logo = RootGrid.ActualTheme == ElementTheme.Dark ? "pilotdeck-logo-white.png" : "pilotdeck-logo.png";
    ...
    private async Task SendComposerAsync()
    {
        ...
        if (session is null)
        {
            ...
            BeginChatScrollTransaction(ChatScrollMutation.Send);
            RenderChatSurface();
            return;
        }
        PersistWorkspaceSnapshot();
        ...
        catch (Exception ex)
        {
            ...
            BeginChatScrollTransaction(ChatScrollMutation.Send);
            RenderChatSurface(refreshSidebar: true);
            return;
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            BeginChatScrollTransaction(ChatScrollMutation.MarkdownFinalized);
        ...
        finally
        {
            ...
            PersistSessionMessages(request.SessionId);
            runCts.Dispose();
            if (isCurrentRun)
            {
                _agentRunCts = null;
    ...
    private static ResolvedAgentModel ModelFromRequest(AgentRequest request)
    {
        ...
        return new ResolvedAgentModel(
            values.GetValueOrDefault("provider.providerId", "pilotdeck"),
    ...
    private async Task<PermissionRecord> ShowPermissionRequestOnUiAsync(PermissionRequest request, CancellationToken cancellationToken)
    {
        ...
        BeginChatScrollTransaction(ChatScrollMutation.PermissionAdded);
        RenderChatSurface();
        ...
        bool remember = false)
    {
        ...
        BeginChatScrollTransaction(ChatScrollMutation.PermissionResolved);
        RenderChatSurface(refreshSidebar: true);
    }
    ...
    private string AgentComposerStatus()
    {
        ...
        var mode = ComposerRunModeLabel(State.ComposerRunMode);
        ...
        return $"{mode} / {model.DisplayLabel} - ~/.pd/config.yaml";
    }
    ...
    private async Task RefreshProviderPreflightAsync()
    {
        ...
        {
            _lastProviderPreflight = ProviderPreflightResult.Failure(null, ex.Message, "Fix ~/.pd/config.yaml or Settings.");
        }
    }
    ...
    {
        var mode = ProjectCreationMode.Existing;
        var step = 0;
        ...
        {
            PlaceholderText = T("project.displayName"),
            Style = (Style)Application.Current.Resources["V2TextBoxStyle"],
        ...
        {
            PlaceholderText = T("project.workspacePath"),
            Style = (Style)Application.Current.Resources["V2TextBoxStyle"],
        ...
        var progressHost = new ContentControl
        {
            HorizontalAlignment = HorizontalAlignment.Stretch,
            HorizontalContentAlignment = HorizontalAlignment.Left,
        };
        ...
        {
            Text = T("project.createSubtitle"),
            FontSize = 12,
            Foreground = Brush("V2MutedForegroundBrush"),
            TextWrapping = TextWrapping.Wrap,
        };
        var stepHeadingText = new TextBlock
        {
            FontSize = 15,
            FontWeight = Microsoft.UI.Text.FontWeights.SemiBold,
            Foreground = Brush("V2ForegroundBrush"),
        };
        TextBlock StepHeadingTextBlock() => new()
        {
            Text = step switch
            {
                0 => T("project.chooseWorkspaceType"),
                1 => T("project.configureWorkspace"),
                _ => T("project.review"),
            },
            FontSize = 15,
            FontWeight = Microsoft.UI.Text.FontWeights.SemiBold,
        ...
        FrameworkElement WizardField(string label, FrameworkElement field) => new StackPanel
        {
            Spacing = 5,
            Children =
            {
        ...
        Button TypeCard(ProjectCreationMode cardMode, string title, string detail, string iconKey)
        {
            var selected = mode == cardMode;
            var button = new Button
            {
                HorizontalAlignment = HorizontalAlignment.Stretch,
                HorizontalContentAlignment = HorizontalAlignment.Stretch,
                MinHeight = ProjectCreationWizardMetrics.TypeCardMinHeight,
                Padding = new Thickness(14),
                CornerRadius = new CornerRadius(8),
                Background = selected ? Brush("V2ControlActiveBrush") : Brush("V2CardBrush"),
                BorderBrush = selected ? Brush("V2BlueBrush") : Brush("V2BorderBrush"),
                ...
                Content = new Grid
                {
                    ...
                    Children =
                    {
                        new Border
                        {
                            ...
                            Height = 34,
                            CornerRadius = new CornerRadius(7),
                            Background = selected ? Brush("V2ControlActiveBrush") : Brush("V2MutedBrush"),
                            Child = Icon(iconKey, 17, selected ? Brush("V2BlueBrush") : Brush("V2MutedForegroundBrush")),
                        },
                        new StackPanel
                        {
                            Spacing = 5,
                        ...
                        selected
                            ? Icon("CheckCircle", 14, Brush("V2BlueBrush"))
                            : new Border { Width = 14, Height = 14 },
                    },
                },
            };
            Grid.SetColumn((FrameworkElement)((Grid)button.Content).Children[1], 1);
            Grid.SetColumn((FrameworkElement)((Grid)button.Content).Children[2], 2);
            button.Click += (_, _) =>
            {
                mode = cardMode;
                errorText.Text = "";
        ...
        FrameworkElement ChooseTypeStep()
        {
            ...
            cards.ColumnDefinitions.Add(new ColumnDefinition { Width = new GridLength(1, GridUnitType.Star) });
            var existing = TypeCard(ProjectCreationMode.Existing, T("project.openExisting"), T("project.openExistingDetail"), "Folder");
            var createNew = TypeCard(ProjectCreationMode.New, T("project.createNew"), T("project.createNewDetail"), "FolderPlus");
            cards.Children.Add(existing);
            Grid.SetColumn(createNew, 1);
            cards.Children.Add(createNew);
            return new StackPanel
            {
                Width = ProjectCreationWizardMetrics.FormMaxWidth,
                Spacing = 12,
                Children =
                {
        ...
        FrameworkElement ConfigureStep() => new StackPanel
        {
            Width = ProjectCreationWizardMetrics.FormMaxWidth,
            Spacing = 12,
            Children =
            {
                StepHeadingTextBlock(),
                WizardField(T("project.displayName"), nameBox),
                WizardField(T("project.workspacePath"), PathRow()),
            },
        };

        FrameworkElement ReviewLine(string label, string value)
        {
            var row = new Grid
            {
                MinHeight = 36,
            ...
            if (string.Equals(label, T("project.workspacePath"), StringComparison.Ordinal))
            {
        ...
        FrameworkElement ReviewDivider() => new Border
        {
            Height = 1,
            Background = Brush("V2BorderBrush"),
            Opacity = 0.8,
        ...
        FrameworkElement ReviewStep()
        {
            var validation = ProjectCreationWizardPolicy.ValidateWorkspace(_workspaceService, mode, pathBox.Text);
            var resolved = validation.ResolvedPath ?? pathBox.Text.Trim();
            var displayName = string.IsNullOrWhiteSpace(resolved)
                ? nameBox.Text.Trim()
                : ProjectCreationWizardPolicy.DisplayNameOrFolder(nameBox.Text, resolved);
            var typeText = mode == ProjectCreationMode.Existing ? T("project.openExisting") : T("project.createNew");
            return new StackPanel
            {
                Width = ProjectCreationWizardMetrics.FormMaxWidth,
                Spacing = 14,
                Children =
                {
                    ...
                    new Border
                    {
                        ...
                        Child = new StackPanel
                        {
                            ...
                            Children =
                            {
                                ReviewLine(T("project.type"), typeText),
                                ReviewDivider(),
                                ReviewLine(T("project.displayName"), displayName),
                                ReviewDivider(),
                                ReviewLine(T("project.workspacePath"), resolved),
                            },
                        },
                    },
                },
        ...
        bool ValidateCurrentStep(out ProjectCreationValidation validation)
        {
            validation = ProjectCreationWizardPolicy.ValidateWorkspace(_workspaceService, mode, pathBox.Text);
            if (validation.Valid)
            {
                errorText.Text = "";
                return true;
            ...
            errorText.Text = validation.Error ?? T("project.invalidPath");
            return false;
        }

        FrameworkElement ProgressBadge(int index)
        {
            ...
            return new Border
            {
                ...
                Height = 20,
                CornerRadius = new CornerRadius(10),
                Background = active ? Brush("V2BlueBrush") : Brush("V2MutedBrush"),
                BorderBrush = active ? Brush("V2BlueBrush") : Brush("V2BorderBrush"),
                BorderThickness = new Thickness(1),
                Child = completed
                    ? Icon("CheckCircle", 10, Brush("V2InverseForegroundBrush"))
                    : new TextBlock
                    {
                        Text = $"{index + 1}",
                        FontSize = 10,
                        FontWeight = Microsoft.UI.Text.FontWeights.SemiBold,
                        Foreground = active ? Brush("V2InverseForegroundBrush") : Brush("V2MutedForegroundBrush"),
                        HorizontalAlignment = HorizontalAlignment.Center,
                        VerticalAlignment = VerticalAlignment.Center,
                    },
            };
        ...
        void RenderProgress()
        {
            var labels = new[] { T("project.type"), T("project.configureWorkspace"), T("project.review") };
            var row = new StackPanel
            {
                Orientation = Orientation.Horizontal,
                Spacing = 0,
            ...
            for (var index = 0; index < labels.Length; index++)
            {
                row.Children.Add(new StackPanel
                {
                    ...
                    Children =
                    {
                        ...
                        new TextBlock
                        {
                            Text = labels[index],
                            FontSize = 11.5,
                            FontWeight = index == step ? Microsoft.UI.Text.FontWeights.Medium : Microsoft.UI.Text.FontWeights.Normal,
                            Foreground = index <= step ? Brush("V2ForegroundBrush") : Brush("V2MutedForegroundBrush"),
                            VerticalAlignment = VerticalAlignment.Center,
                            TextTrimming = TextTrimming.CharacterEllipsis,
                        },
                    },
                ...
                if (index < labels.Length - 1)
                {
                    row.Children.Add(new Border
                    {
                        Width = 56,
                        Height = 1.5,
                        Background = index < step ? Brush("V2BlueBrush") : Brush("V2BorderBrush"),
                        Opacity = index < step ? 0.42 : 0.88,
            ...
            progressHost.Content = row;
        }

        void RenderWizardStep()
        {
            stepText.Text = $"{step + 1} / 3";
            stepHeadingText.Text = step switch
            {
                0 => T("project.chooseWorkspaceType"),
                1 => T("project.configureWorkspace"),
                _ => T("project.review"),
            };
            detailText.Text = step switch
            {
                0 => T("project.createSubtitle"),
                1 => mode == ProjectCreationMode.Existing ? T("project.openExistingDetail") : T("project.createNewDetail"),
                _ => T("project.reviewDetail"),
            };
            contentHost.Content = step switch
            {
                0 => ChooseTypeStep(),
                1 => ConfigureStep(),
                _ => ReviewStep(),
            };
            backButton.Content = step == 0 ? T("common.cancel") : T("project.back");
            cancelButton.Visibility = step == 0 ? Visibility.Collapsed : Visibility.Visible;
            nextButton.Content = step == 2 ? T("common.create") : T("project.next");
            backButton.IsEnabled = !isCreating;
            cancelButton.IsEnabled = !isCreating;
            nextButton.IsEnabled = !isCreating;
            RenderProgress();
        }

        void CloseWizard(bool result) => completion.TrySetResult(result);
        ...
        {
            if (!isCreating) CloseWizard(false);
        };
        nextButton.Click += (_, _) =>
        {
            ...
            if (!ValidateCurrentStep(out var validation))
            {
                return;
            }
            ...
            try
            {
                isCreating = true;
                RenderWizardStep();
                ProjectCreationWizardPolicy.EnsureDirectory(mode, validation.ResolvedPath!);
                var displayName = ProjectCreationWizardPolicy.DisplayNameOrFolder(nameBox.Text, validation.ResolvedPath!);
                ...
                _uiSettings = _uiSettings with
                {
                    ...
                    LastProjectId = project.Id.ToString(),
                };
                ChatLines.Clear();
                PersistUiSettings();
                CloseWizard(true);
        ...
        {
            Width = ProjectCreationWizardMetrics.MaxWidth,
            MaxWidth = ProjectCreationWizardMetrics.MaxWidth,
            HorizontalAlignment = HorizontalAlignment.Center,
            VerticalAlignment = VerticalAlignment.Center,
        ...
        void UpdateModalBounds()
        {
            var available = ProjectCreationOverlayRoot.ActualWidth > 0 ? ProjectCreationOverlayRoot.ActualWidth : RootGrid.ActualWidth;
            if (available > 0)
            {
                modal.Width = Math.Min(ProjectCreationWizardMetrics.MaxWidth, Math.Max(320, available - 48));
            }
        }

        SizeChangedEventHandler? sizeHandler = null;
        sizeHandler = (_, _) => UpdateModalBounds();
        ProjectCreationOverlayRoot.SizeChanged += sizeHandler;

        var root = new Grid { HorizontalAlignment = HorizontalAlignment.Stretch };
        root.RowDefinitions.Add(new RowDefinition { Height = new GridLength(ProjectCreationWizardMetrics.HeaderHeight) });
        root.RowDefinitions.Add(new RowDefinition { Height = GridLength.Auto });
        root.RowDefinitions.Add(new RowDefinition { Height = GridLength.Auto });
        root.RowDefinitions.Add(new RowDefinition { Height = new GridLength(ProjectCreationWizardMetrics.FooterHeight) });
        ...
        header.Children.Add(new Border
        {
            ...
            Height = 32,
            CornerRadius = new CornerRadius(8),
            Background = Brush("V2ControlActiveBrush"),
            Child = Icon("FolderPlus", 15, Brush("V2BlueBrush")),
            VerticalAlignment = VerticalAlignment.Center,
        });
        ...
        var closeButton = new Button
        {
            ...
            BorderBrush = Transparent,
            CornerRadius = new CornerRadius(8),
            Content = Icon("X", 13, Brush("V2MutedForegroundBrush")),
            VerticalAlignment = VerticalAlignment.Center,
        };
        ...
        progressRow.Children.Add(progressHost);
        Grid.SetColumn(stepText, 1);
        progressRow.Children.Add(stepText);
        Grid.SetRow(progressRow, 1);
        root.Children.Add(progressRow);
        var topDivider = new Border
        {
            Height = 1,
            Background = Brush("V2BorderBrush"),
            Opacity = 0.8,
        ...
        Grid.SetRow(bottomDivider, 2);
        root.Children.Add(bottomDivider);
        ...
        RenderWizardStep();
        ProjectCreationOverlayHost.Content = overlay;
        ProjectCreationOverlayRoot.Visibility = Visibility.Visible;
        UpdateModalBounds();
        var shouldRender = await completion.Task;
        if (sizeHandler is not null) ProjectCreationOverlayRoot.SizeChanged -= sizeHandler;
        ProjectCreationOverlayRoot.Visibility = Visibility.Collapsed;
        ProjectCreationOverlayHost.Content = null;
        if (shouldRender)
        {
            RenderAll();
        }
    ...
    private async Task CreateProjectContentDialogAsync()
    {
        var mode = ProjectCreationMode.Existing;
        var step = 0;
        var nameBox = new TextBox
        ...
        Button TypeCard(ProjectCreationMode cardMode, string title, string detail, string iconKey)
        {
            ...
            var button = new Button
            {
                ...
                Content = new Grid
                {
                    ...
                    Children =
                    {
                        Icon(iconKey, 18, selected ? Brush("V2BlueBrush") : Brush("V2MutedForegroundBrush")),
                        new StackPanel
                        {
                            Spacing = 4,
                            Children =
                        ...
                        Icon(selected ? "CheckCircle" : "CircleGauge", 15, selected ? Brush("V2BlueBrush") : Brush("V2MutedForegroundBrush")),
                    },
                },
            };
            Grid.SetColumn((FrameworkElement)((Grid)button.Content).Children[1], 1);
        ...
        FrameworkElement ReviewLine(string label, string value) => new Grid
        {
            ColumnSpacing = 14,
            ColumnDefinitions =
            {
        ...
        FrameworkElement ReviewStep()
        {
            ...
            var displayName = string.IsNullOrWhiteSpace(resolved)
                ...
                : ProjectCreationWizardPolicy.DisplayNameOrFolder(nameBox.Text, resolved);
            var typeText = mode == ProjectCreationMode.Existing ? T("project.openExisting") : T("project.createNew");
            var typeLine = ReviewLine(T("project.type"), typeText);
            var nameLine = ReviewLine(T("project.displayName"), displayName);
            var pathLine = ReviewLine(T("project.workspacePath"), resolved);
            Grid.SetColumn((FrameworkElement)((Grid)typeLine).Children[1], 1);
            Grid.SetColumn((FrameworkElement)((Grid)nameLine).Children[1], 1);
            Grid.SetColumn((FrameworkElement)((Grid)pathLine).Children[1], 1);
            return new StackPanel
            {
                Width = ProjectCreationWizardMetrics.FormMaxWidth,
                Spacing = 12,
        ...
        void RenderWizardStep()
        {
            ...
            contentHost.Content = step switch
            {
                0 => new StackPanel
                {
                    ...
                    Children =
                    {
                        TypeCard(ProjectCreationMode.Existing, T("project.openExisting"), T("project.openExistingDetail"), "Folder"),
                        TypeCard(ProjectCreationMode.New, T("project.createNew"), T("project.createNewDetail"), "FolderPlus"),
                    },
                },
                1 => ConfigureStep(),
                _ => ReviewStep(),
        ...
        var header = new Grid
        {
            Padding = new Thickness(24, 16, 24, 10),
            ColumnSpacing = 12,
        };
        header.ColumnDefinitions.Add(new ColumnDefinition { Width = new GridLength(1, GridUnitType.Star) });
        header.ColumnDefinitions.Add(new ColumnDefinition { Width = GridLength.Auto });
    ...
    private async Task DeleteProjectAsync(WorkspaceProject project)
    {
        ...
        {
            State.MessagesBySession.Remove(session.Id);
            try
            {
                _workspaceSessionStore.DeleteMessages(session.Id);
            }
            catch (IOException)
            {
            }
    ...
    private async Task ShowSettingsAsync(SettingsMainTab? initialTab = null)
    {
        ...
        var generalWorkspace = Box(draft.GeneralWorkspacePath, @"C:\Users\you\PilotDeck\general");
        var databasePath = Box(draft.RuntimeSettings?.DatabasePath ?? State.Settings.RuntimeSettings?.DatabasePath ?? "", @"%LOCALAPPDATA%\PilotDeck\pilotdeck.db");
        ...
        var gatewayHome = Box(gateway.Home, @"%USERPROFILE%\.pd");
        ...
        var providerId = Box(draft.Providers?.FirstOrDefault()?.Id ?? "pilotdeck", "pilotdeck");
        ...
        string CurrentMcpPath() =>
            string.Equals(mcpScope, "project", StringComparison.OrdinalIgnoreCase) && !string.IsNullOrWhiteSpace(mcpProjectRoot)
                ? Path.Combine(mcpProjectRoot, ".pd", "mcp.json")
        ...
        mcpProjectRow = SettingsMenuRow("Folder", T("sidebar.projects"), T("settings.mcp.projectScopeDetail"), mcpProjectPicker);
        ...
        RefreshMcpView();
        var configContent = new ContentControl
        {
            Content = configPanel,
            HorizontalAlignment = HorizontalAlignment.Stretch,
            HorizontalContentAlignment = HorizontalAlignment.Stretch,
        ...
        var backButton = new Button
        {
            ...
            BorderBrush = Transparent,
            CornerRadius = new CornerRadius(8),
            Content = IconText("ChevronLeft", T("common.back"), 14),
        };

        FrameworkElement? mainPanel = null;
        FrameworkElement MainSettingsPanel() => mainPanel ??= SettingsPanel(
            SettingsSection(T("settings.section.basics"), "",
                SettingsNavigationRow("FileCog", T("settings.tabs.config"), T("settings.config.mainDetail"), () => ShowSettingsPage(SettingsOverlayPage.Config))),
            SettingsSection(T("settings.section.application"), "",
            ...
            SettingsSection(T("settings.section.workflow"), "",
                SettingsNavigationRow("MessageSquarePlus", T("settings.workflow.chatInput"), T("settings.workflow.chatInputDetail"), () => ShowSettingsPage(SettingsOverlayPage.Behavior)),
                SettingsNavigationRow("Code", T("settings.appearance.codeEditor"), T("settings.appearance.codeEditorDetail"), () => ShowSettingsPage(SettingsOverlayPage.CodeEditor))),
            SettingsSection(T("settings.section.advanced"), "",
                SettingsNavigationRow("Database", T("settings.mcp.servers"), T("settings.mcp.serversDetail"), () => ShowSettingsPage(SettingsOverlayPage.Mcp)),
                SettingsNavigationRow("Shield", T("settings.tabs.permissions"), T("settings.permissions.navDetail"), () => ShowSettingsPage(SettingsOverlayPage.Permissions))));
        ...
        void ShowSettingsPage(SettingsOverlayPage page)
        {
            ...
            {
                SettingsOverlayPage.Behavior => T("settings.workflow.chatInput"),
                ...
                SettingsOverlayPage.Mcp => T("settings.mcp.servers"),
                _ => T("settings.title"),
            };
        ...
        var modal = BuildSettingsOverlay(content, pageTitle, backButton, errorText, saveStatus, completion, SaveAsync, null);
        ...
        var settingsWindow = CreateSettingsWindow(modal, completion);
        ...
        FrameworkElement? boundsRoot)
    {
        ...
        void UpdateBounds()
        {
            var overlayWidth = boundsRoot?.ActualWidth > 0 ? boundsRoot.ActualWidth : SettingsWindowMetrics.DefaultWidth;
            var overlayHeight = boundsRoot?.ActualHeight > 0 ? boundsRoot.ActualHeight : SettingsWindowMetrics.DefaultHeight;
            var metrics = new SettingsOverlayMetrics(overlayWidth, overlayHeight);
            modal.Width = overlayWidth > 0 ? overlayWidth : metrics.Width;
            modal.Height = overlayHeight > 0 ? overlayHeight : metrics.Height;
            content.Width = metrics.ContentWidth;
            content.MaxWidth = metrics.ContentWidth;
            ...
            {
                header.Width = metrics.ColumnWidth;
                header.Padding = new Thickness(metrics.SideInset, 48, metrics.SideInset, 18);
            }
            if (body is not null) body.Width = metrics.ColumnWidth;
            if (contentHost is not null)
            {
                contentHost.Padding = new Thickness(metrics.SideInset, 0, metrics.SideInset, 18);
            }
            if (footer is not null)
            {
                footer.Width = metrics.ColumnWidth;
                footer.Padding = new Thickness(metrics.SideInset, 10, metrics.SideInset, 20);
            }
        }
        ...
        if (boundsRoot is not null)
        {
            boundsRoot.SizeChanged += sizeHandler;
        }
        ...
        closeButton.Margin = new Thickness(0, 0, 14, 0);
        closeButton.Click += (_, _) =>
        {
            if (sizeHandler is not null && boundsRoot is not null) boundsRoot.SizeChanged -= sizeHandler;
    ...
    private Window CreateSettingsWindow(FrameworkElement content, TaskCompletionSource<bool> completion)
    {
    ...
    private void ConfigureSettingsWindow(Window settingsWindow)
    {
        ...
        var scale = WindowRasterizationScale();
        var availableWidth = RootGrid.ActualWidth > 0 ? Math.Min(RootGrid.ActualWidth, SettingsWindowMetrics.DefaultWidth) : SettingsWindowMetrics.DefaultWidth;
        var availableHeight = RootGrid.ActualHeight > 0 ? Math.Min(RootGrid.ActualHeight, SettingsWindowMetrics.DefaultHeight) : SettingsWindowMetrics.DefaultHeight;
        var metrics = new SettingsWindowMetrics(availableWidth, availableHeight);
        var width = (int)Math.Ceiling(metrics.Width * scale);
        var height = (int)Math.Ceiling(metrics.Height * scale);
        appWindow.Resize(new SizeInt32(width, height));

        if (_hwnd != IntPtr.Zero)
        {
            _ = SetWindowLongPtr(hwnd, GwlHwndParent, _hwnd);
            if (GetWindowRect(_hwnd, out var ownerRect))
            {
                var ownerWidth = Math.Max(0, ownerRect.Right - ownerRect.Left);
                var ownerHeight = Math.Max(0, ownerRect.Bottom - ownerRect.Top);
                appWindow.Move(new PointInt32(
                    ownerRect.Left + Math.Max(0, ownerWidth - width) / 2,
                    ownerRect.Top + Math.Max(0, ownerHeight - height) / 2));
            }
        }
    }
        ...
        IReadOnlyDictionary<string, string> providerSecrets)
    {
        ...
        void RefreshProviderChoices()
        {
            var ids = providerRows
                ...
                .ToList();
            if (ids.Count == 0) ids.Add("pilotdeck");
        ...
        {
            var id = Box(provider.Id, "pilotdeck");
        ...
        addProvider.Click += (_, _) =>
        {
            ...
            AddProviderRow(new NativeProviderEntry($"provider{index}", ProviderApiType.OpenAIChat, "", $"pilotdeck-provider-provider{index}", []));
            RefreshProviderChoices();
        };
        var addEntry = SmallButton(T("settings.config.addModelEntry"));
        addEntry.Click += (_, _) =>
        {
            var providerId = providerRows.FirstOrDefault()?.IdBox.Text.Trim() ?? "pilotdeck";
        ...
        return new SettingsModelsEditorState
        {
            ...
            {
                var id = NativeSettingsIds.Normalize(row.IdBox.Text, index == 0 ? "pilotdeck" : $"provider{index + 1}");
                var account = string.IsNullOrWhiteSpace(row.InitialSecretAccount)
                    ? $"pilotdeck-provider-{id}"
            ...
            ReadModelEntries = () => modelRows.Select((row, index) =>
            {
                var provider = row.ProviderBox.SelectedItem is ComboBoxItem { Tag: string tag }
                    ? tag
                    : providerRows.FirstOrDefault()?.IdBox.Text.Trim() ?? "pilotdeck";
            ...
            WriteSecretsAsync = async store =>
            {
                foreach (var row in providerRows)
                {
                    ...
                    var id = NativeSettingsIds.Normalize(row.IdBox.Text, "pilotdeck");
                    var account = string.IsNullOrWhiteSpace(row.InitialSecretAccount)
                        ? $"pilotdeck-provider-{id}"
                        : row.InitialSecretAccount;
    ...
    private FrameworkElement WindowChromeIcon(string iconKey) => new Border
    {
        Width = WindowControlMetrics.HitSize,
        Height = WindowControlMetrics.HitSize,
        CornerRadius = new CornerRadius(6),
        Background = Transparent,
        Child = Icon(iconKey, WindowControlMetrics.IconSize, Brush("V2SecondaryForegroundBrush")),
        HorizontalAlignment = HorizontalAlignment.Center,
        VerticalAlignment = VerticalAlignment.Center,
    };
    ...
    private void PersistWorkspaceSnapshot()
    {
        try
        {
            _workspaceSessionStore.SaveSnapshot(State);
        }
        catch (Exception ex) when (ex is IOException or UnauthorizedAccessException or JsonException)
        {
            State.StatusLine = $"Workspace state persistence failed: {ex.Message}";
        }
    }

    private void PersistSessionMessages(string? sessionId = null)
    {
        var targetSessionId = sessionId ?? State.SelectedSessionId;
        if (targetSessionId is null) return;
        if (!State.MessagesBySession.TryGetValue(targetSessionId, out var messages)) return;
        try
        {
            _workspaceSessionStore.SaveMessages(targetSessionId, messages);
            PersistWorkspaceSnapshot();
        }
        catch (Exception ex) when (ex is IOException or UnauthorizedAccessException or JsonException)
        {
            State.StatusLine = $"Session persistence failed: {ex.Message}";
        }
    }

    private void PersistRoutingUsage()
    {
        try
        {
            _workspaceSessionStore.SaveRoutingUsage(State.RoutingUsage);
        }
        catch (Exception ex) when (ex is IOException or UnauthorizedAccessException or JsonException)
        {
            State.StatusLine = $"Routing persistence failed: {ex.Message}";
        }
    }
