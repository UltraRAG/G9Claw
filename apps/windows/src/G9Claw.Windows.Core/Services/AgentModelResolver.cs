...
public static class AgentModelResolver
{
    ...
    public static ProviderPreflightResult Preflight(AppSettings settings, string? apiKey)
    {
        ...
        {
            return ProviderPreflightResult.Failure(null, ex.Message, "Fix the provider baseUrl in ~/.g9claw/config.yaml or Settings.");
        }
    ...
    public static ProviderPreflightResult Preflight(ResolvedAgentModel model, string? apiKey)
    {
        ...
        {
            return ProviderPreflightResult.Failure(model, "Provider API key is missing.", "Save a real apiKey in Settings or ~/.g9claw/config.yaml so it can be stored in DPAPI.");
        }
