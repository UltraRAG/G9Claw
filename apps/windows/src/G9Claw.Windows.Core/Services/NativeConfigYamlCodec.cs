...
public static class NativeConfigYamlCodec
{
    ...
    public static NativeConfigYamlParseResult ApplyYaml(AppSettings current, string yaml)
    {
        ...
        foreach (var (id, value) in providersYaml)
        {
            ...
            var normalizedId = NativeSettingsIds.Normalize(id, providers.Count == 0 ? "g9claw" : $"provider{providers.Count + 1}");
            var secretAccount = GetString(provider, "secretAccount", $"g9claw-provider-{normalizedId}");
