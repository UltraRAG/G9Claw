...
namespace PilotDeck.Windows.Tests;
...
public sealed class ParityLogicTests
{
    ...
    public void WebV2PrimaryTabsMatchDesktopOrder()
    {
        ...
        Assert.Equal([AppTab.Chat, AppTab.Skills], AppTabCatalog.GeneralProjectTabs);
    ...
    [Fact]
    public void HeaderToolSwitcherTabsDependOnProjectContext()
    {
        var now = DateTimeOffset.UtcNow;
        var general = new WorkspaceProject(Guid.NewGuid(), "general", "general", @"C:\Users\tester\Workspace\general", [], [], [], [], now, now);
        var project = new WorkspaceProject(Guid.NewGuid(), "project", "Project", @"C:\Users\tester\Workspace\project", [], [], [], [], now, now);

        Assert.Equal(
            [AppTab.Chat, AppTab.Skills],
            AppTabCatalog.ToolSwitcherTabsForProject(general).Select(tab => tab.Tab).ToList());
        Assert.Equal(
            [AppTab.Chat, AppTab.Files, AppTab.Skills, AppTab.Dashboard, AppTab.Memory, AppTab.AlwaysOn],
            AppTabCatalog.ToolSwitcherTabsForProject(project).Select(tab => tab.Tab).ToList());
        Assert.Equal(AppTab.Chat, AppTabCatalog.ClampToolSwitcherTab(AppTab.Memory, general));
        Assert.Equal(AppTab.Memory, AppTabCatalog.ClampToolSwitcherTab(AppTab.Memory, project));
        Assert.Equal(AppTab.Chat, AppTabCatalog.ClampToolSwitcherTab(AppTab.Preview, project));
    }

    [Fact]
    public void CoreDescriptorsMatchNativeMacAppModels()
    {
        Assert.Equal("pilotdeck", SessionProvider.PilotDeck.Id());
        Assert.Equal("PilotDeck", SessionProvider.PilotDeck.DisplayName());
        Assert.Equal("Cursor", SessionProvider.Cursor.DisplayName());
        Assert.True(SessionProvider.PilotDeck.IsNativeAvailable());
        ...
        Assert.Equal("\u5141\u8bb8\u6240\u6709\u6743\u9650", ComposerPermissionMode.BypassPermissions.Label());
    ...
    [Fact]
    public void ComposerSendEligibilityAllowsLocalProviderErrors()
    {
        var session = Session("s1", SessionProvider.PilotDeck, DateTimeOffset.UtcNow);

        var missingProvider = ComposerSendEligibilityPolicy.Evaluate(
            session,
            hasSelectedProject: true,
            composerText: "帮我优化一下这个网页",
            attachmentCount: 0,
            isAgentBusy: false,
            isAgentModelConfigured: false);

        Assert.True(missingProvider.CanSubmit);
        Assert.False(missingProvider.CanRunAgent);
        Assert.True(missingProvider.OpensLocalConfigurationError);
        Assert.Equal(ComposerSendEligibilityPolicy.ProviderNotConfigured, missingProvider.Reason);

        Assert.False(ComposerSendEligibilityPolicy.Evaluate(
            session,
            hasSelectedProject: true,
            composerText: "",
            attachmentCount: 0,
            isAgentBusy: false,
            isAgentModelConfigured: false).CanSubmit);

        Assert.False(ComposerSendEligibilityPolicy.Evaluate(
            session with { IsReadOnly = true },
            hasSelectedProject: true,
            composerText: "continue",
            attachmentCount: 0,
            isAgentBusy: false,
            isAgentModelConfigured: true).CanSubmit);
    }
    ...
    public void ConfigAndPreferenceHelpersMatchMacStoragePolicies()
    {
        ...
        Assert.Equal("pilotdeck", PermissionsExportDefaults.Source);
        Assert.Equal("pilotdeck-permissions-2026-05-29.json", PermissionsExportDefaults.Filename(date));
        Assert.Equal("alwaysOn", PilotDeckConfigSection.AlwaysOn.Id());
        Assert.Equal("Always-On", PilotDeckConfigSection.AlwaysOn.Label());
        Assert.Equal("Raw YAML", PilotDeckConfigSection.Raw.Label());
    ...
    public void ChatEmptyStateTitleMatchesMacProjectWelcomePrompt()
    {
        var now = DateTimeOffset.UtcNow;
        var general = Project("general", "general", now);
        var project = Project("demo", "PilotDeck", now);
        ...
        Assert.Equal("Where should we move PilotDeck forward today?", string.Format(english.T(ChatEmptyStatePresentation.ProjectTitleKey), "PilotDeck"));
        Assert.Equal(
            "\u4ece\u300c\u539f\u795e\u300d\u5f00\u59cb\uff0c\u4eca\u5929\u63a8\u8fdb\u54ea\u4e00\u5757\uff1f",
    ...
    public void GeneralProjectEntryPresentationMatchesMacPickerPolicy()
    {
        ...
        Assert.True(GeneralProjectEntryPresentation.ShouldRender(general, AppTab.Chat, SidebarSection.General, hasMessages: false, isReadOnlyBackgroundSession: false));
        Assert.False(GeneralProjectEntryPresentation.ShouldRender(general, AppTab.Chat, SidebarSection.Projects, hasMessages: false, isReadOnlyBackgroundSession: false));
        Assert.False(GeneralProjectEntryPresentation.ShouldRender(general, AppTab.Skills, SidebarSection.General, hasMessages: false, isReadOnlyBackgroundSession: false));
        Assert.False(GeneralProjectEntryPresentation.ShouldRender(general, AppTab.Chat, SidebarSection.General, hasMessages: true, isReadOnlyBackgroundSession: false));
        Assert.False(GeneralProjectEntryPresentation.ShouldRender(general, AppTab.Chat, SidebarSection.General, hasMessages: false, isReadOnlyBackgroundSession: true));
        Assert.False(GeneralProjectEntryPresentation.ShouldRender(alpha, AppTab.Chat, SidebarSection.General, hasMessages: false, isReadOnlyBackgroundSession: false));
    ...
    public void CodeLineNumberMetricsGrowWithDigitCount()
    {
        ...
        Assert.Equal(3, CodeLineNumberMetrics.LineCount("one\rtwo\r"));
    ...
    public void CodeEditorScrollStabilityMetricsEnableMinimapDraggingAndThrottleViewport()
    {
        ...
        Assert.True(EditableCodeEditorMetrics.FillsAvailableViewport);
        Assert.Equal(18, EditableCodeEditorMetrics.LineHeight(14));
        Assert.Equal(36, EditableCodeEditorMetrics.ScrollOffsetForLine(3, 14));

        var visible = EditableCodeEditorMetrics.VisibleLineRange(36, 90, 14, 200);
        Assert.Equal(3, visible.Start.Value);
        Assert.Equal(9, visible.End.Value);

        var clamped = EditableCodeEditorMetrics.VisibleLineRange(9_999, 90, 14, 20);
        Assert.Equal(20, clamped.Start.Value);
        Assert.Equal(21, clamped.End.Value);
    }

    [Fact]
    public void EditableCodeEditorMetricsExposeLineNumbersMinimapAndFullHeightChrome()
    {
        Assert.True(EditableCodeEditorMetrics.FillsAvailableViewport);
        Assert.True(EditableCodeEditorMetrics.UsesInternalVerticalScroll);
        Assert.True(EditableCodeEditorMetrics.LineNumbersApplyToEditableEditor);
        Assert.True(EditableCodeEditorMetrics.MinimapAppliesToEditableEditor);
        Assert.True(EditableCodeEditorMetrics.SyntaxHighlightingAppliesToEditableEditor);
        Assert.Equal(48, EditableCodeEditorMetrics.MinimumLineGutterWidth);
        Assert.Equal(CodeMinimapModel.Width, EditableCodeEditorMetrics.MinimapWidth);
        Assert.Equal(10, EditableCodeEditorMetrics.GutterRightPadding);
        Assert.Equal(13.5f, EditableCodeEditorMetrics.RichEditLineSpacingPoints(14));
        Assert.True(EditableCodeEditorMetrics.CanApplyInteractiveSyntaxHighlighting(40_000, 1_500));
        Assert.False(EditableCodeEditorMetrics.CanApplyInteractiveSyntaxHighlighting(80_000, 1_500));
        Assert.False(EditableCodeEditorMetrics.CanApplyInteractiveSyntaxHighlighting(40_000, 3_000));

        var contentWidth = EditableCodeEditorMetrics.EditableTextWidth(
            availableWidth: 980,
            lineNumbersVisible: true,
            minimapVisible: true,
            lineCount: 12_000);

        Assert.Equal(CodeLineNumberMetrics.RulerWidth(12_000), EditableCodeEditorMetrics.LineGutterWidth(true, 12_000));
        Assert.Equal(CodeMinimapModel.Width, EditableCodeEditorMetrics.MinimapColumnWidth(true));
        Assert.True(contentWidth < 980);
        Assert.True(contentWidth > 760);
        Assert.Equal(980, EditableCodeEditorMetrics.EditableTextWidth(980, false, false, 12_000));
    }

    [Fact]
    public void FileTreeRowsStretchAndDockInlineActionsRight()
    {
        Assert.True(FileTreeRowMetrics.RowsStretchToContainer);
        Assert.True(FileTreeRowMetrics.InlineActionsDockRight);
        Assert.Equal(28, FileTreeRowMetrics.RowHeight);
        Assert.Equal(6, FileTreeRowMetrics.LeadingPadding(0));
        Assert.Equal(42, FileTreeRowMetrics.LeadingPadding(2));
        Assert.Equal(24, FileTreeRowMetrics.ActionColumnWidth);
    }

    [Fact]
    public void EditorDirtyStateSeparatesModelWritesFromUserDrafts()
    {
        var state = EditorDirtyState.ForFile("index.html", "one");

        var externalBeforeUserEdit = state.Apply(EditorDirtyChangeKind.ExternalWrite, "index.html", "one\ntwo");
        Assert.False(externalBeforeUserEdit.IsDirty);
        Assert.Equal("one\ntwo", externalBeforeUserEdit.DraftText);

        var userEdit = externalBeforeUserEdit.Apply(EditorDirtyChangeKind.UserEdit, "index.html", "one\ntwo\nlocal");
        Assert.True(userEdit.IsDirty);

        var externalAfterUserEdit = userEdit.Apply(EditorDirtyChangeKind.ExternalWrite, "index.html", "one\ntwo\nmodel");
        Assert.True(externalAfterUserEdit.IsDirty);
        Assert.Equal("one\ntwo\nlocal", externalAfterUserEdit.DraftText);
        Assert.Equal("one\ntwo\nmodel", externalAfterUserEdit.BaselineText);

        var saved = externalAfterUserEdit.Apply(EditorDirtyChangeKind.Save, "index.html", externalAfterUserEdit.DraftText);
    ...
    [Fact]
    public void InteractiveCardsUseOpaqueSurfacesAndBoundedSpinners()
    {
        Assert.True(InteractiveCardMetrics.UsesOpaqueV2Surface);
        Assert.Equal(18, InteractiveCardMetrics.SpinnerHostSize);
        Assert.Equal(12, InteractiveCardMetrics.SpinnerRingSize);
        Assert.True(InteractiveCardMetrics.SpinnerRingSize < InteractiveCardMetrics.SpinnerHostSize);
        Assert.True(InteractiveCardMetrics.PermissionActionMinWidth >= 88);
    }

    [Fact]
    public void ProjectCreationWizardValidationMatchesExistingAndNewModes()
    {
        var root = Path.Combine(Path.GetTempPath(), "pilotdeck-project-wizard-tests", Guid.NewGuid().ToString("N"));
        ...
        try
        {
            ...
            var existingValidation = ProjectCreationWizardPolicy.ValidateWorkspace(service, ProjectCreationMode.Existing, existing);
            var missingExistingValidation = ProjectCreationWizardPolicy.ValidateWorkspace(service, ProjectCreationMode.Existing, missing);
            var newValidation = ProjectCreationWizardPolicy.ValidateWorkspace(service, ProjectCreationMode.New, missing);
            var outsideValidation = ProjectCreationWizardPolicy.ValidateWorkspace(
                service,
                ProjectCreationMode.New,
                Path.Combine(Path.GetTempPath(), "outside-workspace"));

            Assert.True(existingValidation.Valid);
            Assert.False(missingExistingValidation.Valid);
            Assert.Contains("does not exist", missingExistingValidation.Error);
            Assert.True(newValidation.Valid);
            Assert.False(Directory.Exists(missing));

            ProjectCreationWizardPolicy.EnsureDirectory(ProjectCreationMode.New, newValidation.ResolvedPath!);

            Assert.True(Directory.Exists(missing));
            Assert.False(outsideValidation.Valid);
            Assert.Equal("missing", ProjectCreationWizardPolicy.DisplayNameOrFolder("", newValidation.ResolvedPath!));
            Assert.Equal("Custom", ProjectCreationWizardPolicy.DisplayNameOrFolder(" Custom ", newValidation.ResolvedPath!));
        }
        finally
        {
    ...
    public void PlanConfirmationCardUsesBluePlanAreaAndVerticalChoicesLikeMac()
    {
        ...
        Assert.Equal(220, PlanConfirmationCardMetrics.MaxPlanHeightForViewport(560));
        Assert.InRange(PlanConfirmationCardMetrics.MaxPlanHeightForViewport(820), 360, 420);
        Assert.Equal(460, PlanConfirmationCardMetrics.MaxPlanHeightForViewport(1200));
        Assert.Equal(42, PlanConfirmationCardMetrics.PlanBadgeTopPadding);
    ...
    public void WebV2SidebarSessionRowsFlattenProviderBucketsByActivity()
    {
        ...
        var project = new WorkspaceProject(
            ...
            "Demo",
            @"C:\Users\tester\demo",
            [Session("g9", SessionProvider.PilotDeck, now.AddMinutes(-30))],
        ...
        Assert.Equal([SessionProvider.Codex, SessionProvider.Cursor, SessionProvider.PilotDeck, SessionProvider.Gemini], rows.Select(row => row.Provider));
    }

    [Fact]
    public void WebV2SidebarSessionIndicatorPrioritizesProcessingThenUnread()
    {
        var session = Session("s1", SessionProvider.PilotDeck, DateTimeOffset.UtcNow, SessionState.Failed);
    ...
    public void LegacyConfigLoaderReadsDefaultProviderSettings()
    {
        ...
        Assert.Equal("~/PilotDeck/general", snapshot.GeneralWorkspacePath);
    }
    ...
    public void NativeConfigServiceResolvesRouterDefaultEntry()
    {
        ...
        gateway:
          runtimePaths:
            generalCwd: C:\Users\tester\PilotDeck\general
        models:
          providers:
            pilotdeck:
              type: openai-chat
    ...
    public void NativeConfigServiceUsesDefaultSecretAccountForBuiltInProviders()
    {
        ...
        models:
          ...
          entries:
            default:
              ...
              name: qwen3.6-27b
        """;
        var pilotdeckValues = NativeConfigService.ScalarMap(pilotdeckYaml);

        var pilotdeckProvider = NativeConfigService.ProviderConfigFor("default", pilotdeckValues);
        Assert.NotNull(pilotdeckProvider);
        Assert.Equal(ProviderConfig.Empty.SecretAccount, pilotdeckProvider!.SecretAccount);
        ...
        Assert.Equal("pilotdeck-provider-custom", customProvider!.SecretAccount);
    }
    ...
    public void AgentModelResolverUsesYamlAgentMainModelAndPreflight()
    {
        ...
        Assert.Equal("pilotdeck", resolved.ProviderId);
    ...
    public void AgentModelResolverDoesNotSilentlyFallbackWhenMainModelIsMissing()
    {
        var settings = AppState.NormalizeSettings(AppSettings.Defaults(@"C:\Users\tester") with
        {
            Providers =
            [
                new NativeProviderEntry("pilotdeck", ProviderApiType.OpenAIChat, "http://example.local/v1", "pilotdeck-provider-pilotdeck", [])
            ],
            ModelEntries =
            [
                new NativeModelEntry("default", "pilotdeck", "qwen3.6-27b", 160000)
            ],
    ...
    public void GeneralWorkspacePathFallsBackWhenConfigIsRelative()
    {
        ...
        Assert.Equal(PathHelpers.NormalizeFullPath(@"C:\Users\tester\PilotDeck\general"), AppState.NormalizeGeneralWorkspacePath("general", home));
        Assert.Equal(PathHelpers.NormalizeFullPath(@"C:\Users\tester\PilotDeck\general"), AppState.NormalizeGeneralWorkspacePath("  ", home));
    ...
    [Fact]
    public void DefaultsUsePilotDeckPaths()
    {
        const string home = @"C:\Users\tester";
        ...
        Assert.Equal(Path.Combine(home, "PilotDeck", "general"), settings.GeneralWorkspacePath);
        Assert.Equal(Path.Combine(home, "PilotDeck", "general"), runtime.GeneralWorkspacePath);
        ...
        Assert.Equal("PilotDeck", AppPaths.ProductDirectoryName);
    }

    [Fact]
    public void NativeConfigServiceUsesPdDefaultPathAndCopiesLegacyConfigOnce()
    {
        using var temp = new TempWorkspace();
        var home = temp.Root;
        var current = NativeConfigService.DefaultConfigPath(home);
        var legacy = Path.Combine(home, string.Concat(".", "g", "9", "claw"), "config.yaml");

        Directory.CreateDirectory(Path.GetDirectoryName(legacy)!);
        File.WriteAllText(legacy, "models:\n  entries: {}\n", Encoding.UTF8);

        Assert.EndsWith(Path.Combine(".pd", "config.yaml"), current);
        Assert.Equal(current, NativeConfigService.EnsureDefaultConfigMigrated(home));
        Assert.True(File.Exists(current));
        Assert.Equal("models:\n  entries: {}\n", File.ReadAllText(current, Encoding.UTF8));
    ...
    public void ShellWorkingDirectoryRejectsRelativeOrMissingPaths()
    {
        ...
        var missing = Assert.Throws<DirectoryNotFoundException>(() => AgentToolExecutor.ValidatedWorkingDirectory($@"C:\pilotdeck-missing-{Guid.NewGuid():D}"));
    ...
    public void SkillRuntimeEnvironmentMatchesMacRagConfigMapping()
    {
        var environment = SkillRuntimeEnvironment.Build(
            ...
            {
                ["PATH"] = @"C:\Tools",
                ["PILOTDECK_PLUGIN_ROOT"] = @"C:\legacy",
            },
            pluginRoot: @"C:\plugins\pilotdeck-rag-plugin");

        Assert.Equal(@"C:\plugins\pilotdeck-rag-plugin", environment["PILOTDECK_PLUGIN_ROOT"]);
        Assert.Equal("1", environment["PILOTDECK_RAG_ENABLED"]);
        Assert.Equal("0", environment["PILOTDECK_RAG_DISABLE_BUILTIN_WEB_TOOLS"]);
        Assert.Equal("http://local.example", environment["PILOTDECK_RAG_LOCAL_KNOWLEDGE_BASE_URL"]);
        Assert.Equal("local-key", environment["PILOTDECK_RAG_LOCAL_KNOWLEDGE_API_KEY"]);
        Assert.Equal("embed", environment["PILOTDECK_RAG_LOCAL_KNOWLEDGE_MODEL_NAME"]);
        Assert.Equal("http://milvus.example/", environment["PILOTDECK_RAG_LOCAL_KNOWLEDGE_DATABASE_URL"]);
        Assert.Equal("http://milvus.example/", environment["PILOTDECK_RAG_LOCAL_KNOWLEDGE_MILVUS_URI"]);
        Assert.Equal("12", environment["PILOTDECK_RAG_LOCAL_KNOWLEDGE_TOP_K"]);
        Assert.Equal("http://glm.example", environment["PILOTDECK_RAG_GLM_WEB_SEARCH_BASE_URL"]);
        Assert.Equal("glm-key", environment["PILOTDECK_RAG_GLM_WEB_SEARCH_API_KEY"]);
        Assert.Equal("8", environment["PILOTDECK_RAG_GLM_WEB_SEARCH_TOP_K"]);
    ...
    {
        using var temp = new TempWorkspace();
        var pluginRoot = Path.Combine(temp.Root, "Assets", "pilotdeck-rag-plugin");
    ...
    public async Task AgentToolExecutorShellReceivesNativeConfigEnvironment()
    {
        ...
        {"command":"Write-Output $env:PILOTDECK_RAG_ENABLED; Write-Output $env:PILOTDECK_RAG_GLM_WEB_SEARCH_TOP_K"}
        """), context);
    ...
    public async Task AgentToolExecutorTaskShellReceivesNativeConfigEnvironment()
    {
        ...
        {"type":"shell","prompt":"Write-Output $env:PILOTDECK_RAG_ENABLED; Write-Output $env:PILOTDECK_RAG_GLM_WEB_SEARCH_TOP_K","run_in_background":false}
        """), context);
        var background = await executor.ExecuteAsync(new AgentToolCall("task-shell-background-env", "Task", """
        {"type":"shell","prompt":"Write-Output $env:PILOTDECK_RAG_ENABLED","description":"Background env","run_in_background":true}
        """), context);
    ...
    public async Task AgentToolExecutorReadLintsRunsConfiguredCommandLikeMac()
    {
        ...
        var context = new AgentToolExecutionContext(
            ...
            new Dictionary<string, string>
            {
                ["lint.command"] = "Write-Output \"src/file.cs:2:4: warning: env=$env:PILOTDECK_RAG_ENABLED\"; Write-Output \"src/file.cs:3: error: broken\"",
                ["rag.enabled"] = "true",
            });
    ...
    public async Task ProviderClientBuildsMacShapedAttachmentParts()
    {
        ...
        var request = new AgentRequest(
            ...
            new ProviderConfig(SessionProvider.PilotDeck, ProviderApiType.OpenAIChat, "http://provider.local/v1", "test-model", "secret", []),
            "test-key",
            [],
            120000,
            160000,
...
""");
    ...
    public async Task NativeAgentRunnerRejectsNonOpenAIChatApiTypesBeforeThinkingLikeMac()
    {
        ...
        var request = new AgentRequest(
            ...
            "inspect the workspace",
            [],
            new ProviderConfig(SessionProvider.PilotDeck, ProviderApiType.OpenAIResponses, "http://provider.local/v1", "test-model", "secret", []),
            "test-key",
            [],
            120000,
            160000,
    ...
    [Fact]
    public void AskQuestionAnswerCodecBuildsMultiSelectAnswersPayload()
    {
        var choices = new JsonArray { "Visual", "Performance", "  " };
        var updated = AskQuestionAnswerCodec.UpdatedInputJson(
            """{"question":"Which areas?","multi_select":true,"options":["Visual","Performance","A11y"]}""",
            new Dictionary<string, JsonNode?>
            {
                ["Which areas?"] = choices,
                ["Other"] = JsonValue.Create("Keyboard"),
            });

        using var doc = JsonDocument.Parse(updated);
        var answers = doc.RootElement.GetProperty("answers");
        Assert.Equal(["Visual", "Performance"], answers.GetProperty("Which areas?").EnumerateArray().Select(item => item.GetString() ?? "").ToArray());
        Assert.Equal("Keyboard", answers.GetProperty("Other").GetString());
        Assert.Contains("\"Which areas?\"=\"Visual, Performance\"", AskQuestionAnswerCodec.Output(updated));
    }

    [Fact]
    public async Task NativeAgentRunnerParsesAskQuestionMultiSelectAliases()
    {
        using var temp = new TempWorkspace();
        var provider = new MultiSelectAskQuestionProvider();
        var runner = new NativeAgentRunner(providerClient: provider);
        var request = new AgentRequest(
            "session-1",
            temp.Root,
            ...
            "default",
            []);
        var sawMultiSelect = false;
        var options = new NativeAgentRunOptions((permission, _) =>
        {
            var question = Assert.Single(permission.InteractivePayload!.Questions);
            sawMultiSelect = question.MultiSelect;
            Assert.Equal(["Visual", "Performance", "A11y"], question.Options.Select(option => option.Label).ToArray());
            var selected = new JsonArray { "Visual", "Performance" };
            var updated = AskQuestionAnswerCodec.UpdatedInputJson(permission.InputJson, new Dictionary<string, JsonNode?>
            {
        ...
        Assert.True(sawMultiSelect);
        var result = Assert.Single(events.Where(item => item.Kind == AgentEventKind.ToolResult).Select(item => item.ToolResult!));
        Assert.Contains("\"Which areas?\"=\"Visual, Performance\"", result.Output);
        Assert.Contains(events, item => item.Kind == AgentEventKind.ContentDelta && item.Text == "continued");
    }
    ...
    [Fact]
    public void AgentToolSchemasIncludePilotDeckCodeCoreTools()
    {
    ...
    public void NativeAgentRuntimeParsesFallbackToolsArrayAndLsGlobLikeMac()
    {
        const string text = """
        {
          "tools": [
            ...
            {
              "id": "skill-call",
              "skill": "pilotdeck-rag:rag-research",
              "args": "native parity"
            }
          ]
        }
        ...
        Assert.Equal("pilotdeck-rag:rag-research", skillInput.GetProperty("skill").GetString());
    ...
    [Fact]
    public void NativeAgentRuntimeParsesPilotDeckInvokeFallbackLikeMac()
    {
        const string skillInvoke = """
        <invoke name="skill">
        <parameter name="skill">pilotdeck-rag:rag-research</parameter>
        <parameter name="args">DARPA &amp; autonomous &quot;systems&quot;</parameter>
        </invoke>
        """;
        const string directRagInvoke = """
        <invoke name="pilotdeck-rag:glm-web-search">
        ...
        Assert.Equal("pilotdeck-rag:rag-research", skillJson.GetProperty("skill").GetString());
        Assert.Equal("DARPA & autonomous \"systems\"", skillJson.GetProperty("args").GetString());
        Assert.Equal("Skill", directRag.Name);
        Assert.Equal("pilotdeck-rag:glm-web-search", directRagJson.GetProperty("skill").GetString());
    ...
    public void LegacySearchAndWeatherCallsNormalizeToGLMSkill()
    {
        ...
        Assert.Contains("pilotdeck-rag:glm-web-search", search.Call.InputJson);
    ...
    public async Task LegacyWebFetchReturnsMacDisabledGuidance()
    {
        ...
        Assert.Equal("WebFetch is disabled. Use Skill with pilotdeck-rag:rag-research for source-grounded web evidence.", result.Output);
    }
    ...
    public void GitServiceCoversBranchDiffDiscardAndUntrackedDelete()
    {
        ...
        var first = service.Commit(temp.Root, "initial", ["README.md"], "PilotDeck", "pilotdeck@example.local");
    ...
    public async Task AppSettingsStoreLoadsOlderJsonWithNewDefaults()
    {
        ...
        await File.WriteAllTextAsync(settingsFile, """
        {
          "providerConfig": {
            ...
            "baseUrl": "http://example.local/v1",
            "model": "qwen",
            "secretAccount": "pilotdeck-default-provider"
          },
          "workspacesRoot": "C:\\Users\\tester",
          "generalWorkspacePath": "C:\\Users\\tester\\PilotDeck\\general",
          "apiTimeoutMs": 90000,
          "contextWindow": 160000,
    ...
    public void RoutingUsageAggregatorComputesDashboardTotals()
    {
        ...
        var records = new[]
        {
            new RoutingUsageRecord("s1", "Demo", "default", "PilotDeck", "cheap", "default", 1000, 500, 0.001m, 0.004m, now),
            new RoutingUsageRecord("s2", "Demo", "default", "PilotDeck", "cheap", "default", 2000, 1000, 0.002m, 0.006m, now.AddMinutes(-1)),
        };
...
""");
    ...
    [Fact]
    public void WorkspaceSessionStoreRestoresProjectsMessagesAndRouting()
    {
        using var temp = new TempWorkspace();
        var now = DateTimeOffset.UtcNow;
        var state = AppState.CreateDefault();
        state.Projects.Clear();
        var project = Project("demo", "Demo", now);
        state.Projects.Add(project);
        state.SelectProject(project);
        var session = state.CreateSessionForSelectedProject("Persist me")!;
        state.AppendMessage(new ChatMessage(
            Guid.NewGuid(),
            session.Id,
            SessionProvider.PilotDeck,
            ChatRole.User,
            [ChatBlock.FromText("hello")],
            now,
            IsStreaming: true,
            null));
        state.RoutingUsage.Add(new RoutingUsageRecord(
            session.Id,
            "Demo",
            "default",
            "pilotdeck",
        ...
        var store = new WorkspaceSessionStore(temp.Root);
        store.SaveSnapshot(state);
        store.SaveMessages(session.Id, state.CurrentMessages);
        store.SaveRoutingUsage(state.RoutingUsage);

        var restored = AppState.CreateDefault();
        store.RestoreInto(restored, store.LoadSnapshot()!);
        restored.RoutingUsage.AddRange(store.LoadRoutingUsage());

        Assert.Equal(project.Id, restored.SelectedProjectId);
        Assert.Equal(session.Id, restored.SelectedSessionId);
        Assert.Equal("Demo", restored.SelectedProject!.DisplayName);
        var message = Assert.Single(restored.CurrentMessages);
        Assert.Equal("hello", message.PlainText);
        Assert.False(message.IsStreaming);
        Assert.Equal("gpt-test", Assert.Single(restored.RoutingUsage).Model);
    }
    ...
    public void AppStateExposesMacParitySessionActivityAndUiState()
    {
        ...
        state.PilotDeckConfigText = "runtime:";
        ...
        Assert.Equal("runtime:", state.PilotDeckConfigText);
    ...
    [Fact]
    public void ContextBudgetPresenterLocalizesTouchedStatusText()
    {
        var normal = ContextBudgetPresenter.FromBudget(new TokenBudget(20, 100), chinese: true);
        var attention = ContextBudgetPresenter.FromBudget(new TokenBudget(61, 100), chinese: true);
        var warning = ContextBudgetPresenter.FromBudget(new TokenBudget(81, 100), chinese: true);
        var recovering = ContextBudgetPresenter.FromBudget(new TokenBudget(96, 100), chinese: true);
        var compacting = ContextBudgetPresenter.FromBudget(new TokenBudget(50, 100), "compact stage 2", 1, chinese: true);

        Assert.Equal("\u4e0a\u4e0b\u6587\u6b63\u5e38", ContextBudgetPresenter.LevelLabel(normal.Level, chinese: true));
        Assert.Equal("\u4e0a\u4e0b\u6587\u9700\u5173\u6ce8", ContextBudgetPresenter.LevelLabel(attention.Level, chinese: true));
        Assert.Equal("\u4e0a\u4e0b\u6587\u8b66\u544a", ContextBudgetPresenter.LevelLabel(warning.Level, chinese: true));
        Assert.Equal("\u4e0a\u4e0b\u6587\u6062\u590d\u4e2d", ContextBudgetPresenter.LevelLabel(recovering.Level, chinese: true));
        Assert.Equal("\u4e0a\u4e0b\u6587\u538b\u7f29\u4e2d", ContextBudgetPresenter.LevelLabel(compacting.Level, chinese: true));
        Assert.DoesNotContain("Context", warning.Detail);
        Assert.Contains("\u4e0a\u4e0b\u6587\u8b66\u544a", warning.Detail);
    }
    ...
    public void NativeI18nResolvesLanguageAndFallsBack()
    {
        ...
        Assert.Equal("Allow all permissions", new StringCatalog(AppLanguage.English).T("chat.composer.bypassPermissions"));
        Assert.Equal("允许所有权限", new StringCatalog(AppLanguage.ChineseSimplified).T("chat.composer.bypassPermissions"));
    ...
    [Fact]
    public void NativeI18nTouchedUiKeysDoNotFallBackToRawKeys()
    {
        var english = new StringCatalog(AppLanguage.English);
        var chinese = new StringCatalog(AppLanguage.ChineseSimplified);
        var touchedKeys = new[]
        {
            ...
            "project.back",
            "common.back",
            "settings.section.basics",
            "settings.workflow.chatInput",
            "settings.mcp.servers",
            "settings.mcp.projectScopeDetail",
        };

        foreach (var key in touchedKeys)
        {
            Assert.NotEqual(key, english.T(key));
            Assert.NotEqual(key, chinese.T(key));
        }

        Assert.Equal("Agent", english.T("chat.composer.runMode.agent"));
        Assert.Equal("Plan", english.T("chat.composer.runMode.plan"));
        Assert.Equal("\u667a\u80fd\u4f53", chinese.T("chat.composer.runMode.agent"));
        Assert.Equal("\u8ba1\u5212", chinese.T("chat.composer.runMode.plan"));
        Assert.Equal("Attach folder", english.T("chat.composer.attachFolder"));
        Assert.Equal("\u6dfb\u52a0\u6587\u4ef6\u5939", chinese.T("chat.composer.attachFolder"));
        Assert.Equal("\u5e38\u9a7b", chinese.T("tabs.alwaysOn"));
        Assert.Equal("\u667a\u80fd\u4f53", chinese.T("settings.config.agents"));
        Assert.Equal("\u5e38\u9a7b", chinese.T("settings.config.alwaysOn"));
        Assert.Equal("\u7f51\u5173", chinese.T("settings.config.gateway"));
        Assert.Equal("\u542f\u7528\u5e38\u9a7b", chinese.T("settings.config.alwaysOnEnabled"));
    }
    ...
    public void HeaderLayoutMetricsReservesCaptionButtons()
    {
        ...
        Assert.Equal(1050, metrics.CaptionReserveStartX);
        Assert.True(metrics.AllowsCustomDragAt(1049));
        Assert.False(metrics.AllowsCustomDragAt(1050));
        Assert.True(metrics.IsInCaptionButtonReserve(1190));
    }

    [Fact]
    public void HeaderMetricsAllowFullWidthNavigationWithCustomLeftWindowControls()
    {
        var metrics = new HeaderLayoutMetrics(1200, 0);

        Assert.Equal(12, metrics.EffectiveRightPadding);
        Assert.Equal(1188, metrics.CaptionReserveStartX);
        Assert.True(metrics.AllowsCustomDragAt(1187));
        Assert.False(metrics.IsInCaptionButtonReserve(1180));
        Assert.Equal(
            [WindowControlCommand.Minimize, WindowControlCommand.ToggleMaximize, WindowControlCommand.Close],
            WindowControlMetrics.CommandOrder);
        Assert.True(WindowControlMetrics.IsWithinControls(18, 16));
        Assert.False(WindowControlMetrics.IsWithinControls(120, 16));
    }
    ...
    public void SettingsOverlayMetricsUseLogicalWindowSize()
    {
        ...
        var wide = new SettingsOverlayMetrics(1920, 1080);
        var minimum = new SettingsOverlayMetrics(980, 640);
        var tiny = new SettingsOverlayMetrics(320, 200);

        Assert.Equal(1440, desktop.Width);
        Assert.Equal(1440, desktop.ColumnWidth);
        Assert.Equal(40, desktop.SideInset);
        Assert.Equal(1360, desktop.ContentWidth);
        Assert.Equal(810, desktop.Height);
        Assert.Equal(1920, wide.ColumnWidth);
        Assert.Equal(40, wide.SideInset);
        Assert.Equal(1840, wide.ContentWidth);
        Assert.Equal(980, minimum.Width);
        Assert.Equal(40, minimum.SideInset);
        Assert.Equal(900, minimum.ContentWidth);
        Assert.Equal(576, minimum.Height);
        Assert.Equal(320, tiny.Width);
        Assert.Equal(40, tiny.SideInset);
        Assert.Equal(240, tiny.ContentWidth);
        Assert.True(tiny.Height <= 200);
    }

    [Fact]
    public void SettingsWindowMetricsUseNarrowOwnedWindowDefaults()
    {
        var defaultWindow = new SettingsWindowMetrics(1920, 1080);
        var compact = new SettingsWindowMetrics(720, 500);

        Assert.Equal(920, defaultWindow.Width);
        Assert.Equal(720, defaultWindow.Height);
        Assert.Equal(920, defaultWindow.ContentWidth);
        Assert.Equal(40, defaultWindow.SideInset);
        Assert.Equal(SettingsWindowMetrics.MinWidth, compact.Width);
        Assert.Equal(SettingsWindowMetrics.MinHeight, compact.Height);
    }

    [Fact]
    public void SettingsRouterAndSidebarMetricsStretchAcrossAvailableWindow()
    {
        foreach (var width in new[] { 720d, 1280d, 1920d })
        {
            var settings = new SettingsOverlayMetrics(width, 900);
            Assert.Equal(width, settings.Width);
            Assert.Equal(settings.SideInset, (settings.Width - settings.ContentWidth) / 2);
        }

        Assert.True(RoutingDashboardLayoutMetrics.RootStretchesToWindow);
        Assert.Equal(720, RoutingDashboardLayoutMetrics.ContentWidth(720));
        Assert.Equal(1280, RoutingDashboardLayoutMetrics.ContentWidth(1280));
        Assert.Equal(RoutingDashboardLayoutMetrics.MaxContentWidth, RoutingDashboardLayoutMetrics.ContentWidth(1920));
        Assert.Equal(300, RoutingDashboardLayoutMetrics.SideInset(1920));

        Assert.Equal(SidebarToggleButtonDock.SidebarHeaderRight, SidebarAnimationMetrics.ToggleButtonDock(sidebarVisible: true));
        Assert.Equal(SidebarToggleButtonDock.MainHeaderLeft, SidebarAnimationMetrics.ToggleButtonDock(sidebarVisible: false));
        Assert.Equal(248, SidebarAnimationMetrics.WidthAt(248, 0, 0));
        Assert.Equal(0, SidebarAnimationMetrics.WidthAt(248, 0, 1));
        Assert.InRange(SidebarAnimationMetrics.WidthAt(248, 0, 0.5), 0, 124);
    }
    ...
    public void ChatScrollPresenterSticksOnlyWhenUserIsNearBottom()
    {
        ...
        Assert.Equal(1180, ChatScrollPresenter.TargetOffset(history, extentHeight: 1700, viewportHeight: 520, forceToBottom: true));

        Assert.True(PlanExecutionTransitionPolicy.ShouldForceScrollToBottomOnPermissionChange(
            history,
        ...
        Assert.True(PlanExecutionTransitionPolicy.ShouldForceScrollToBottomOnPermissionChange(
            ...
            autoScrollToBottom: true));
        Assert.False(PlanExecutionTransitionPolicy.ShouldForceScrollToBottomOnPermissionChange(
            history,
            isResolvingPermission: false,
            autoScrollToBottom: true));
        Assert.Equal(ChatRunMode.Agent, PlanExecutionTransitionPolicy.ComposerModeAfterExitPlan("agent", ChatRunMode.Plan));
        Assert.Equal(ChatRunMode.Plan, PlanExecutionTransitionPolicy.ComposerModeAfterExitPlan("plan", ChatRunMode.Plan));
    }

    [Fact]
    public void ChatScrollTransactionsForceBottomOnlyForUserVisibleFlowChanges()
    {
        Assert.Equal(new ChatScrollTransaction(true, false), ChatScrollTransactionPolicy.ForMutation(ChatScrollMutation.Send, userWasAtBottom: false));
        Assert.Equal(new ChatScrollTransaction(true, false), ChatScrollTransactionPolicy.ForMutation(ChatScrollMutation.PlanExecuted, userWasAtBottom: true));
        Assert.Equal(new ChatScrollTransaction(true, false), ChatScrollTransactionPolicy.ForMutation(ChatScrollMutation.PermissionResolved, userWasAtBottom: false));
        Assert.Equal(new ChatScrollTransaction(true, false), ChatScrollTransactionPolicy.ForMutation(ChatScrollMutation.PermissionAdded, userWasAtBottom: true));
        Assert.Equal(new ChatScrollTransaction(false, true), ChatScrollTransactionPolicy.ForMutation(ChatScrollMutation.PermissionAdded, userWasAtBottom: false));
        Assert.Equal(new ChatScrollTransaction(false, true), ChatScrollTransactionPolicy.ForMutation(ChatScrollMutation.LocalComposerChange, userWasAtBottom: true));
    }
    ...
    [Fact]
    public void MarkdownPresentationPreservesInlineBoundariesAndTaskLists()
    {
        var blocks = MarkdownPresentation.Parse("""
            中文**加粗**`code`文本
        ...
        var paragraph = blocks.First(block => block.Kind == MarkdownBlockKind.Paragraph);
        Assert.Equal("中文加粗code文本", string.Concat(paragraph.Inlines.Select(inline => inline.Text)));
        Assert.DoesNotContain("中文 加粗", string.Join("", paragraph.Inlines.Select(inline => inline.Text)));

        static string InlineText(IEnumerable<MarkdownInlinePresentation> inlines) =>
            string.Concat(inlines.Select(inline => inline.Text));

        var list = blocks.First(block => block.Kind == MarkdownBlockKind.List);
        Assert.Equal(new bool?[] { true, false }, list.ListTaskStates!.ToArray());
        Assert.Equal(["完成", "待办"], list.ListItems!.Select(InlineText).ToArray());

        Assert.Contains(blocks, block => block.Kind == MarkdownBlockKind.ThematicBreak);
        var table = blocks.First(block => block.Kind == MarkdownBlockKind.Table);
        Assert.True(table.Table!.HasHeader);
        Assert.Equal(["列", "值"], table.Table.Rows[0].Select(inline => inline.Text).ToArray());
        Assert.Equal(["一", "二"], table.Table.Rows[1].Select(inline => inline.Text).ToArray());
    }
...
router:
  ...
  tokenStats:
    savingsBaselineModel: pilotdeck,qwen3.6-27b
gateway:
  enabled: false
  home: C:\Users\tester\.pd\gateway
  channels:
    telegram:
      enabled: false
      token: gateway-token
""";
    ...
    public async Task AgentToolExecutorTaskWorktreeIsolationMatchesMacRuntime()
    {
        ...
        using (var isolatedJson = JsonDocument.Parse(isolated.Output))
        {
            ...
            Assert.Contains("pilotdeck-worktrees", worktree);
    ...
    public async Task AgentToolExecutorTaskWorktreeSubagentsUseIsolatedWorkspaceLikeMacRuntime()
    {
        ...
        Assert.Contains("pilotdeck-worktrees", isolatedRequest.WorkspaceRoot);
        ...
        Assert.Contains("pilotdeck-worktrees", attempts[0].GetProperty("worktree").GetString());
    ...
    private sealed class MultiSelectAskQuestionProvider : IProviderClient
    {
        public async IAsyncEnumerable<ProviderStreamEvent> StreamAsync(
            AgentRequest request,
            [EnumeratorCancellation] CancellationToken cancellationToken = default)
        {
                      ...
                      "questions": [
                        {
                          ...
                          "multi_select": true,
                          "choices": ["Visual", "Performance", "A11y"]
                        }
                      ]
                    }
                    """));
            ...
            yield return new ProviderStreamEvent(ProviderStreamEventKind.ContentDelta, Text: "continued");
            yield return new ProviderStreamEvent(ProviderStreamEventKind.Done);
        }
    }
    ...
    private static void InitializeGitRepository(string cwd)
    {
        ...
        RunGit(cwd, "config", "user.name", "PilotDeck Tests");
    ...
    {
        public string Root { get; } = Path.Combine(Path.GetTempPath(), $"pilotdeck-win-tests-{Guid.NewGuid():D}");
