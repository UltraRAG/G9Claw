---
name: g9claw-local-knowledge
description: Search G9Claw's configured local knowledge base for durable domain evidence and internal context.
...
`g9claw-rag:local-knowledge` as a direct tool name.

If explicitly invoking this skill, use:

```json
{"skill":"g9claw-rag:local-knowledge","args":"<query>"}
```
...
`G9CLAW_RAG_LOCAL_KNOWLEDGE_TOP_K`, which is exported from
`rag.localKnowledge.defaultTopK` in `~/.g9claw/config.yaml`.

Optional filters must be a JSON object:

```bash
python3 ${G9CLAW_PLUGIN_ROOT}/scripts/local_knowledge_search.py --query "<query>" --filters-json '{"domain":"military"}'
```
