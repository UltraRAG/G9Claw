#!/usr/bin/env python3
"""Call the configured GLM web search API for G9Claw RAG v1."""
...
def search_glm_web(
    ...
    *,
    ...
    base_url = _env("G9CLAW_RAG_GLM_WEB_SEARCH_BASE_URL")
    api_key = _env("G9CLAW_RAG_GLM_WEB_SEARCH_API_KEY")
    default_top_k = _int_value(_env("G9CLAW_RAG_GLM_WEB_SEARCH_TOP_K"), 8)
    timeout_seconds = _int_value(_env("G9CLAW_RAG_GLM_WEB_SEARCH_TIMEOUT_SECONDS"), DEFAULT_TIMEOUT_SECONDS)
    ...
    if not base_url:
        return _error(query, "missing_config", "G9CLAW_RAG_GLM_WEB_SEARCH_BASE_URL is not configured.")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Search GLM web search for G9Claw RAG.")
