...
struct AlwaysOnView: View {
    ...
    private func startDiscovery(context: WorkspaceContext) {
        ...
        Inspect the workspace, identify useful background maintenance or monitoring work, then create or update Always-On discovery plan files under `.g9claw/always-on/`.
        """
        do {
...
private struct CodeEditorWithChrome: View {
    ...
    var body: some View {
        HStack(spacing: 0) {
            ...
            if showMinimap {
                CodeMinimapView(
                    model: CodeMinimapModel(text: text, visibleLineRange: visibleLineRange),
                    onSeekLine: { line in
...
private struct CodeMinimapView: View {
    ...
    var body: some View {
        GeometryReader { proxy in
            Canvas { context, size in
                ...
                context.fill(
                    ...
                    with: .color(DesignTokens.accent.opacity(0.14))
                )
                context.stroke(
                    Path(roundedRect: viewportRect, cornerRadius: 4),
                    with: .color(DesignTokens.accent.opacity(0.36)),
                    lineWidth: 1
                )
            }
            .contentShape(Rectangle())
        ...
        .background(DesignTokens.neutral50.opacity(0.54))
...
private struct FileContentTextEditor: NSViewRepresentable {
    ...
    final class Coordinator: NSObject, NSTextViewDelegate {
        ...
        func scrollToLine(_ requestedLine: Int) {
            ...
            let documentHeight = scrollView.documentView?.bounds.height ?? 0
            let maxY = max(0, documentHeight - scrollView.contentView.bounds.height)
            let target = NSPoint(x: scrollView.contentView.bounds.origin.x, y: min(maxY, targetY))
