#!/usr/bin/env python3
"""Call the configured G9Claw local knowledge search API.

The API contract is fixed by G9Claw v1:
POST {G9CLAW_RAG_LOCAL_KNOWLEDGE_DATABASE_URL}
{
  "text": string,
  "top_k": number
}
...
def search_local_knowledge(
    ...
    *,
    ...
    base_url = _env("G9CLAW_RAG_LOCAL_KNOWLEDGE_BASE_URL")
    api_key = _env("G9CLAW_RAG_LOCAL_KNOWLEDGE_API_KEY")
    database_url = _env("G9CLAW_RAG_LOCAL_KNOWLEDGE_DATABASE_URL") or _env("G9CLAW_RAG_LOCAL_KNOWLEDGE_MILVUS_URI")
    model_name = _env("G9CLAW_RAG_LOCAL_KNOWLEDGE_MODEL_NAME")
    default_top_k = _int_value(_env("G9CLAW_RAG_LOCAL_KNOWLEDGE_TOP_K"), 8)
    timeout_seconds = _int_value(_env("G9CLAW_RAG_LOCAL_KNOWLEDGE_TIMEOUT_SECONDS"), DEFAULT_TIMEOUT_SECONDS)

    if _env("G9CLAW_RAG_ENABLED", "0") in {"0", "false", "False", "no", "NO"}:
        return _error(query, "rag_disabled", "G9Claw RAG is disabled. Set rag.enabled: true in ~/.g9claw/config.yaml.")
    if not base_url and not database_url:
        return _error(
            query,
            "missing_config",
            "G9CLAW_RAG_LOCAL_KNOWLEDGE_BASE_URL or G9CLAW_RAG_LOCAL_KNOWLEDGE_DATABASE_URL is not configured.",
        )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Search G9Claw local knowledge.")
