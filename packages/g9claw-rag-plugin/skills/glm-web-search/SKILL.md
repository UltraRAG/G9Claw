...
# G9Claw GLM Web Search

Use this skill to search the configured Z.AI / GLM web search service. This
replaces the built-in G9Claw `WebFetch` and `WebSearch` tools for search
tasks in G9Claw.

This is a skill loaded through the built-in `Skill` tool. Do not call
`g9claw-rag:glm-web-search` as a direct tool name.

If explicitly invoking this skill, use:

```json
{"skill":"g9claw-rag:glm-web-search","args":"<query>"}
```
...
```bash
python3 ${G9CLAW_PLUGIN_ROOT}/scripts/glm_web_search.py --query "<query>"
```

Do not pass `--top-k` by default. The script uses
`G9CLAW_RAG_GLM_WEB_SEARCH_TOP_K`, which is exported from
`rag.glmWebSearch.defaultTopK` in `~/.g9claw/config.yaml`.

Optional freshness and domain constraints:

```bash
python3 ${G9CLAW_PLUGIN_ROOT}/scripts/glm_web_search.py --query "<query>" --freshness-days 30
python3 ${G9CLAW_PLUGIN_ROOT}/scripts/glm_web_search.py --query "<query>" --allowed-domain example.com
python3 ${G9CLAW_PLUGIN_ROOT}/scripts/glm_web_search.py --query "<query>" --blocked-domain example.com
```
