---
name: g9claw-rag-research
description: Combine G9Claw local knowledge and GLM web search evidence for research, intelligence, and source-grounded generation tasks.
...
allowed-tools:
  - "Bash(python3 ${G9CLAW_PLUGIN_ROOT}/scripts/local_knowledge_search.py:*)"
  - "Bash(python3 ${G9CLAW_PLUGIN_ROOT}/scripts/glm_web_search.py:*)"
---
...
`g9claw-rag:rag-research` as a direct tool name.

If explicitly invoking this skill, use:

```json
{"skill":"g9claw-rag:rag-research","args":"<research query>"}
```

Do not use built-in `WebFetch` or `WebSearch` for search. Use the scripts below
so all retrieval goes through G9Claw-controlled APIs and citations.

## Retrieval Flow

1. Search local knowledge first for stable domain context:
   ```bash
   python3 ${G9CLAW_PLUGIN_ROOT}/scripts/local_knowledge_search.py --query "<query>"
   ```

2. Identify gaps that require current public evidence, then search GLM web:
   ```bash
   python3 ${G9CLAW_PLUGIN_ROOT}/scripts/glm_web_search.py --query "<gap query>"
   ```
...
configured default from `G9CLAW_RAG_*_TOP_K`, which is exported from
`~/.g9claw/config.yaml`. Only pass `--top-k <n>` when the user explicitly asks
